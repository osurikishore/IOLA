...
!-------------------------------------------------------------------------
                ! Gopal's Kludge
                IF(var == 'FH')THEN
                  WRITE(0,*)'This is Gopals Kludge to make force FH to 00'
                  if(buf(1:used) == '01')buf(1:used)='00'
                  if(buf(1:used) == '07')buf(1:used)='06'
                  if(buf(1:used) == '13')buf(1:used)='12'
                  if(buf(1:used) == '19')buf(1:used)='18'
                ENDIF
!--------------------------------------------------------------------------
                outs(o:o+used-1) = buf(1:used)
                o=o+used
!               WRITE(0,*)'ENTRY 3 ', var,' ',buf(1:used)
             endif
...
!<-hsk 2016: for HYCOM v11 
    if ((mod(lead_time,6)==3).or.(mod(lead_time,6)==-3)) then
       list%filetype='archs'
    endif    
!-> hsk
...
!    write(0,*)'LEAD ATIME & FTIME',lead_time,atime,ftime
!    write(0,*)'------------------------FIRST INPUT--------------------------------'
...
!    write(0,*)'WRONG FILE NAMES',list%ainfile,list%aoutfile

!    write(0,*)'------------------------SECOND INPUT--------------------------------'
...
!    write(0,*)'WRONG FILE NAMES',list%binfile,list%boutfile
