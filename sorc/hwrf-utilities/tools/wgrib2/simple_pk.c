...
//     x64, cache line is 64 bytes or 512 bits
//    not quite true as p+5 may not be on cache boundary :(

#pragma omp parallel for private(j,k)
    for (j = 0; j < ((int) n + 511) / 512; j++) {
	k = n - j*512;
	if (k > 512) k = 512;
        flist2bitstream(data + j*512, p + 5 + (j*64)*nbits, k, nbits);
    }
