...
C command line arguments over-ride defaults
      ...
      if (narg .eq. 9) then
          ...
          read(arg,'(i5)') JMAX
          call getarg(8,arg)
          read(arg,'(l)') use_gfs_nemsio
          call getarg(9,arg)
          read(arg,'(l)') use_gfs_netcdf
      endif
!zz      if(use_gfs_nemsio) then
!zz      IMAX=1010
!zz      JMAX=1010
!zz      endif

      if (use_gfs_nemsio .and. use_gfs_netcdf) then
          print*, "Invalid selection of use_gfs_nemsio and 
     &                                   use_gfs_netcdf"
          print*, "use_gfs_nemsio = ", " use_gfs_netcdf = ",
     &              use_gfs_nemsio, use_gfs_netcdf
          print*, "Choose either NEMSIO or NETCDF option"
          stop
      endif
      IM=IM-1
      JM=JM-1
     ...
     & ,JMT,DLMD,DPHD,IMAX,JMAX,use_gfs_nemsio,use_gfs_netcdf)
      stop
      end
