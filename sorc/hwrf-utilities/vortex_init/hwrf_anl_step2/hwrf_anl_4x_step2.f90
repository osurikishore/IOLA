...
! LON & LAT at U,V
      ...
      CALL EARTH_LATLON ( HLAT3,HLON3,VLAT3,VLON3,  & ! lat, lon at H, V points
...
!           *     *     *     *     *     *     *     *     *     * 
      ...
      ALLOCATE ( IIH1(NXT,NYT), JJH1(NXT,NYT)  )
      ALLOCATE ( IIV1(NXT,NYT), JJV1(NXT,NYT)  )
...
!*    Input HLAT1,HLON1,VLAT1,VLON1 in degrees 
      ...
      CALL G2T2H_egrid( IIH1,JJH1,      & !* interp indices of source H points 
           HBWGT1(1,1,1),HBWGT1(1,1,2), & !* interp weights to target H points 
           HBWGT1(1,1,3),HBWGT1(1,1,4), & !* interp weights to target H points
           CENTRAL_LAT,CENTRAL_LON,     & !* central lat/lon of source  grid 
           CURRENT_DOMAIN_ID,           & ! input source domain ID
           DLMD2,DPHD2,                 & !* dlat/dlon, W/S bdry of source grid 
           HLAT3,HLON3,                 & !* lat/lon of H points on source grid 
           HLAT1,HLON1,                 & !* lat/lon of H points on target grid 
           1,NX,1,NY,                   & !*  x/y dimensions of source  grid 
           1,NX,1,NY,                   & !*  x/y dimensions of source  grid 
           1,NXT,1,NYT            )
 
      CALL G2T2V_egrid( IIV1,JJV1,       & !* interp indices of source V points 
           VBWGT1(1,1,1),VBWGT1(1,1,2), & !* interp weights to target V points 
           VBWGT1(1,1,3),VBWGT1(1,1,4), & !* interp weights to target V points 
           CENTRAL_LAT,CENTRAL_LON,     & !* central lat/lon of source  grid
           CURRENT_DOMAIN_ID,           & ! input source domain ID
           DLMD2,DPHD2,                 & !* dlat/dlon, W/S bdry of source grid 
           VLAT3,VLON3,                 & !* lat/lon of V points on target grid 
...
!$omp& private(i,j,n1,IV1,JV1)
      DO J=1,NYT
      CYC_35: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_35
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_35
...
!$omp& private(i,j,i1,j1)
      DO J=1,NYT
      CYC_36: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_36
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_36
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
         USC1(I1,J1) =                                     &
...
!$omp& private(i,j,k,n1,IH1,JH1)
      DO J=1,NYT
      CYC_45: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_45
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_45
         IF (MOD(JJH1(I,J),2) .NE. 0) THEN !* odd J-index on E-grid 
            IH1(1)=IIH1(I,J)    !* 1: left 
            JH1(1)=JJH1(I,J)
            IH1(2)=IIH1(I,J)+1  !* 2: right 
            JH1(2)=JJH1(I,J)
            IH1(3)=IIH1(I,J)
            JH1(3)=JJH1(I,J)-1  !* 3: bottom 
            IH1(4)=IIH1(I,J)
            JH1(4)=JJH1(I,J)+1  !* 4: top 
         ELSE  !* even J-index on E-grid  
            IH1(1)=IIH1(I,J)
            JH1(1)=JJH1(I,J)
            IH1(2)=IIH1(I,J)+1
            JH1(2)=JJH1(I,J)
...
!$omp& private(i,j,k,i1,j1,k1,n,WRK1,WRK2,TENV1,QENV1,W1,W)
      DO J=1,NYT
      CYC_46: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_46
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_46
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
...
!$omp& private(i,j,k,n1,IV1,JV1)
      DO J=1,NYT
      CYC_55: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_55
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_55
         IF (MOD(JJV1(I,J),2) .NE. 0) THEN !* Odd J-index on E-grid
            IV1(1)=IIV1(I,J)    !* 1: left 
            JV1(1)=JJV1(I,J)
            IV1(2)=IIV1(I,J)+1  !* 2: right 
            JV1(2)=JJV1(I,J)
            IV1(3)=IIV1(I,J)+1
            JV1(3)=JJV1(I,J)-1  !* 3: bottom 
            IV1(4)=IIV1(I,J)+1
            JV1(4)=JJV1(I,J)+1  !* 4: top 
         ELSE  !* even J-index on E-grid 
            IV1(1)=IIV1(I,J)
            JV1(1)=JJV1(I,J)
            IV1(2)=IIV1(I,J)+1
...
!$omp& private(i,j,i1,j1)
      DO J=1,NYT
      CYC_56: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_56
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_56
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
...
!$omp& private(i,j,k,i1,j1,k1,N,WRK1,WRK2,W1,W)
      DO J=1,NYT
      CYC_57: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_57
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_57
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
         DO K=1,KMX
            WRK1(K) =                          &
...
!$omp& private(i,j,k,i1,j1,k1,n1,IH1,JH1,PDIF1,FACT1)
      DO J=1,NYT
      CYC_66: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_66
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_66
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
...
!$omp& private(i,j,i1,j1,TENV1,T_OLD,Q_OLD,ESRR)
      DO J=1,NYT
      CYC_67: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_67
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_67
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
         SLP1(I1,J1) = SLP1(I1,J1)                          & 
                   + HBWGT1(I,J,1)*SLP21(I,J,1)             &
...
!$omp& private(i,j,k,n1,IH1,JH1)
      DO J=1,NYT
      CYC_76: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_76
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_76
         IF (MOD(JJH1(I,J),2) .NE. 0) THEN    ! 1,3,5,7
            IH1(1)=IIH1(I,J)
            JH1(1)=JJH1(I,J)
            IH1(2)=IIH1(I,J)+1
...
!$omp& private(i,j,k,i1,j1,k1,n,WRK1,WRK2,WRK3,TENV1,QENV1,Q_OLD,T_OLD,W1,W,ESRR)
      DO J=1,NYT
      CYC_77: DO I=1,NXT
         IF(IIH1(I,J).LT.1.or.IIH1(I,J).GT.NX_1)CYCLE CYC_77
         IF(JJH1(I,J).LT.1.or.JJH1(I,J).GT.NY_1)CYCLE CYC_77
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
...
!$omp& private(i,j,k,n1,IV1,JV1)
      DO J=1,NYT
      CYC_86: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_86
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_86
         IF (MOD(JJV1(I,J),2) .NE. 0) THEN    ! 1,3,5,7
            IV1(1)=IIV1(I,J)
            JV1(1)=JJV1(I,J)
            IV1(2)=IIV1(I,J)+1
...
!$omp& private(i,j,k,i1,j1,k1,n,WRK1,WRK2,W1,W)
      DO J=1,NYT
      CYC_87: DO I=1,NXT
         IF(IIV1(I,J).LT.1.or.IIV1(I,J).GT.NX_1)CYCLE CYC_87
         IF(JJV1(I,J).LT.1.or.JJV1(I,J).GT.NY_1)CYCLE CYC_87
         J1=J+JWMIN1-1+MDY
         I1=I+IWMIN1-1+MDX
         DO K=1,KMX
            WRK1(K) =                          &
