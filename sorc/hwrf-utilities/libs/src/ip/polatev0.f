...
C$$$
      ...
      TYPE INTERPSTORE_T
         ...
         REAL,POINTER:: CROTX(:)=>NULL(),SROTX(:)=>NULL()
         REAL,POINTER:: WXY(:,:,:)=>NULL()
         REAL,POINTER:: SXY(:,:,:)=>NULL(),CXY(:,:,:)=>NULL()
      ...
      INTEGER,POINTER:: NXY(:,:,:)
      REAL,POINTER:: RLATX(:),RLONX(:),CROTX(:),SROTX(:)
      REAL,POINTER:: WXY(:,:,:),CXY(:,:,:),SXY(:,:,:)
      REAL,ALLOCATABLE::DUM1(:),DUM2(:)
      LOGICAL :: FOUND_INTERP
      INTEGER, SAVE :: NEW_INTERP_LOC=-999, IRETX=-1
      INTEGER :: INTERP_LOC
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C  SET PARAMETERS
      IRET=0
      ...
      IF(NEW_INTERP_LOC<0) THEN
         ...
         do i=1,size(interpstore)
            ...
            nullify(interpstore(i)%crotx)
            nullify(interpstore(i)%srotx)
            nullify(interpstore(i)%cxy)
            nullify(interpstore(i)%sxy)
         enddo
      ENDIF
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C  SAVE OR SKIP WEIGHT COMPUTATION
      ...
      DO INTERP_LOC=1,size(interpstore)
         ...
         SXY=>interpstore(i)%SXY
         CXY=>interpstore(i)%CXY
         SROTX=>interpstore(i)%SROTX
         CROTX=>interpstore(i)%CROTX
...
!     Need to make new interpolation information
         ...
         IF(ASSOCIATED(interpstore(INTERP_LOC)%RLATX)) THEN
            ...
            DEALLOCATE(interpstore(i)%CROTX,interpstore(i)%CXY)
            NULLIFY(interpstore(i)%SROTX,interpstore(i)%SXY)
         ENDIF
         i=INTERP_LOC
...
C  ALLOCATE AND SAVE GRID DATA
     ...
     &             NXY(2,2,NO),WXY(2,2,NO),CXY(2,2,NO),SXY(2,2,NO))
          i=interp_loc
          interpstore(i)%RLATX=>RLATX
          interpstore(i)%RLONX=>RLONX
          interpstore(i)%CROTX=>CROTX
          interpstore(i)%SROTX=>SROTX
          interpstore(i)%NXY=>NXY
          interpstore(i)%WXY=>WXY
          interpstore(i)%CXY=>CXY
          interpstore(i)%SXY=>SXY
          NOX=NO
        ENDIF
        IRETX=IRET
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
