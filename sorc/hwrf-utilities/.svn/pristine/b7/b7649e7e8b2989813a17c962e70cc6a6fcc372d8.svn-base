...
1001 continue
    ...
    close(99)
    seconds = int((hsa_grid%hh*3600.0) + (hsa_grid%nn*60.0) + hsa_grid%ss)
    ...
    write(yymmdd,'(i6)') int(hsa_grid%yymmdd(hsa_grid%nz))
    read(yymmdd(1:2),'(i2)') yyyy
    read(yymmdd(3:4),'(i2)') mm
    read(yymmdd(5:6),'(i2)') dd
    hsa_grid%yyyy = 2000 + yyyy
    hsa_grid%mm   = mm
    hsa_grid%dd   = dd
    ...
    rel_julian = julian_day + real(rel_hmsts)/86400.0

    ! Compute local variables

    call time_methods_gregorian_date(hsa_julian,yyyy_hsa,mm_hsa,dd_hsa,   &
         & hh_hsa,nn_hsa,ss_hsa)
    call time_methods_gregorian_date(rel_julian,yyyy_rel,mm_rel,dd_rel,   &
         & hh_rel,nn_rel,ss_rel)
