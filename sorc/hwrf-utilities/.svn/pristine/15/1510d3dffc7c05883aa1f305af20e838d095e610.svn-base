...
size_t fread_file(void *ptr, size_t size, size_t nmemb, struct seq_file *file) {
    if (file->file_type == DISK || file->file_type == PIPE) return fread(ptr, size, nmemb, file->cfile);
...
size_t fwrite_file(const void *ptr, size_t size, size_t nmemb, struct seq_file *file) {
    ...
    if (file->file_type == DISK || file->file_type == PIPE) return fwrite(ptr, size, nmemb, file->cfile);
