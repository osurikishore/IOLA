program products
  ...
  logical :: grads_byteswap, realtime, write_grib
  integer :: ioutlist
...
308 format('Running with ',I0,' OpenMP threads.')
  ...
  if(len_trim(inwrfdiag)>1) then
     if(final_hour+1e-3>min_swath_hours) then
        ...
        write(ioutlist,'(A)') trim(outpre)//'suswind10m.ascii'
...
!        write(ioutlist,'(A)') trim(outpre)//'trak.h360.atcfunix'

        if(write_grib) then
           write(ioutlist,'(A)') trim(outpre)//'swath.grb2'
        endif
...
102    format(I2)
  ...
  subroutine handle_wrfdiag(maxfile)
    ...
    if(write_grib) then
       iostat=-999
       call swath%open_grib2(trim(outpre)//'swath.grb2',iostat)
       if(iostat/=0) then
          write(0,*) 'Cannot open output GRIB2 file.  Abort.'
          stop 101
       endif
    endif
...
319             format(A,': error writing file')
       ...
       if(write_grib .and. itime>1) then
1319      format(A,': write time ',I0)
          print 1319, trim(outpre)//'swath.grb2',itime
          call swath%write_grib2(ierr=iostat)
          if(iostat/=0) then
1320         format(A,': ERROR: cannot write data for time ',I0,' iostat=',I0)
             write(0,1320) trim(outpre)//'swath.grb2',itime,iostat
          endif
       endif
    enddo timeloop
...
102 format( "Model run did not produce track beyond ",I0," hours.  No storm bulletin can be generated")
  ...
  subroutine htcf_and_htcfstats()
    ...
    close(iunit)
    call vmaxmean(htcfs, nhtcf, atcfs, natcf, time_window, hr_interval)
...
2013      format("Need forecasts beyond hour ",I0," to generate htcfstats and resolution files (last hour is ",I0,")")
  ...
  subroutine vmaxmean(htcfs, nhtcf, atcfs, natcf, time_window, hr_interval)
    implicit none
    integer :: nhtcf, natcf
    type(htcf) :: htcfs(nhtcf)
    type(atcf) :: atcfs(natcf)
    type(atcf) :: atcfs_out(natcf)
    integer :: nwindow, ndhours, mm,nn, n, i, ii, ntlast, natcf_total
    real :: time_window, hr_interval
    real :: windout, dt, window
    dt=10./3.
    nwindow=time_window*60./dt/2.
    ndhours=hr_interval*3600./dt
    ntlast=126
       mm=0
       do n=1,natcf
       if ( mod(atcfs(n)%fcsthour,hr_interval) .eq. 0) then
       mm=mm+1
       atcfs_out(mm)%fcsthour=atcfs(n)%fcsthour
       atcfs_out(mm)%lat=atcfs(n)%lat
       atcfs_out(mm)%lon=atcfs(n)%lon
       atcfs_out(mm)%wind=atcfs(n)%wind
       atcfs_out(mm)%pres=atcfs(n)%pres
       atcfs_out(mm)%wind=atcfs(n)%wind
       endif
       enddo
       if (mm .eq. 0) then
       ...
       ntlast=(natcf_total-1)*3
    mm=0  ! htcf index
    nn=0  ! atcf index
       do n=1,nhtcf
       mm=mm+1
       if (mod(mm-1,ndhours) .eq. 0 ) then
           if( mm-nwindow .ge. 1 .and. mm+nwindow .le. nhtcf ) then
           windout=0.0
           ii=0
           do i=-nwindow,nwindow
           ii=ii+1
           windout=windout+htcfs(mm+i)%wval
           enddo
           windout=windout/float(ii)
           else
           windout=htcfs(mm)%wval
           endif
           if ( int(htcfs(mm)%fcstsec/3600.) .le. ntlast ) then
           nn=nn+1
           ! find corresponding fcsthr in actf
           do i=1,natcf
           if ( atcfs(i)%fcsthour .eq. int(htcfs(mm)%fcstsec/3600.+0.01) ) then
           atcfs_out(nn)%fcsthour=atcfs(i)%fcsthour
           atcfs_out(nn)%basin2=atcfs(i)%basin2
           atcfs_out(nn)%storm2=atcfs(i)%storm2
!           atcfs_out(nn)%model4=atcfs(i)%model4
           atcfs_out(nn)%model4="H360"
           atcfs_out(nn)%ymdh=atcfs(i)%ymdh
           atcfs_out(nn)%lat=atcfs(i)%lat
           atcfs_out(nn)%lon=atcfs(i)%lon
           atcfs_out(nn)%pres=atcfs(i)%pres
           endif
           enddo
           atcfs_out(nn)%wind=windout
           endif
       endif
       enddo
       ...
       nn=nn+1
           ! find corresponding fcsthr in actf
           do i=1,natcf
           if ( int(atcfs(i)%fcsthour) .eq. int(htcfs(mm)%fcstsec/3600.+0.01) ) then
           atcfs_out(nn)%fcsthour=atcfs(i)%fcsthour
           atcfs_out(nn)%basin2=atcfs(i)%basin2
           atcfs_out(nn)%storm2=atcfs(i)%storm2
!           atcfs_out(nn)%model4=atcfs(i)%model4
    ...
    print *,'Write H360 file ',trim(outpre)//'trak.h360.atcfunix'
    write(ioutlist,'(A)') trim(outpre)//'trak.h360.atcfunix'
    iunit=get_unit()
    open(iunit,file=trim(outpre)//'trak.h360.atcfunix', &
         form='FORMATTED',status='UNKNOWN')
    call write_atcf(iunit,atcfs_out,natcf_total)
    close(iunit)

1100 format(A2,", ",A2,", ",I10.10,",",A3,", ", &
         A4,", ",I3.3,", ", &
         I3.3,A1", "I4.4,A1,", ",I3.3", "I4.4", "A7)
  end subroutine vmaxmean
  subroutine write_atcf(iunit,atcfs,natcf)
    integer,intent(in) :: iunit,natcf
    type(atcf), intent(in) :: atcfs(natcf)
    integer :: i
    ...
    writeloop: do i=1,natcf
       pns=to_ns(atcfs(i)%lat)
       pew=to_ew(atcfs(i)%lon)

       if ( pns == 'N' .and. pew == 'E') then
       write(iunit,1100) &
            atcfs(i)%basin2,atcfs(i)%storm2,atcfs(i)%ymdh," 03",atcfs(i)%model4,int(atcfs(i)%fcsthour), &
            int(10*atcfs(i)%lat),pns,int(10.*atcfs(i)%lon),pew,int(atcfs(i)%wind+0.5),int(atcfs(i)%pres), "XX,  34,"
       endif
       if ( pns == 'S' .and. pew == 'E') then
       write(iunit,1100) &
            atcfs(i)%basin2,atcfs(i)%storm2,atcfs(i)%ymdh," 03",atcfs(i)%model4,int(atcfs(i)%fcsthour), &
            -int(10*atcfs(i)%lat),pns,int(10.*atcfs(i)%lon),pew,int(atcfs(i)%wind+0.5),int(atcfs(i)%pres), "XX,  34,"
       endif
       if ( pns == 'N' .and. pew == 'W') then
       write(iunit,1100) &
            atcfs(i)%basin2,atcfs(i)%storm2,atcfs(i)%ymdh," 03",atcfs(i)%model4,int(atcfs(i)%fcsthour), &
            int(10*atcfs(i)%lat),pns,-int(10.*atcfs(i)%lon),pew,int(atcfs(i)%wind+0.5),int(atcfs(i)%pres), "XX,  34,"
       endif
       if ( pns == 'S' .and. pew == 'W') then
       write(iunit,1100) &
            atcfs(i)%basin2,atcfs(i)%storm2,atcfs(i)%ymdh," 03",atcfs(i)%model4,int(atcfs(i)%fcsthour), &
            -int(10*atcfs(i)%lat),pns,-int(10.*atcfs(i)%lon),pew,int(atcfs(i)%wind+0.5),int(atcfs(i)%pres), "XX,  34,"
       endif
...
1100 format(A2,", ",A2,", ",I10.10,",",A3,", ", &
         A4,", ",I3.3,", ", &
         I3.3,A1", "I4.4,A1,", ",I3.3", "I4.4", "A8)
  end subroutine write_atcf
  ...
  subroutine namelist_and_tracks(nmlfile,nmloutfile)
    ...
    integer :: want_ymdh, nmlin, i, ide_moad,jde_moad, last, pnr, iunit
    real :: coupler_dt, grace_period
