module interp_module
  ...
  ! SMOOTHER OPERATIONS:
  integer, parameter, public :: &
       SM_MEAN=128, SM_MAX=129, SM_MIN=130, SM_SUM=131
  ! SM_MEAN - spatial mean, unweighted
  ! SM_SUM - sum of values seen
  ! SM_MAX - maximum value seen
  ! SM_MIN - minimum value seen
  ...
  type, extends(interpolator) :: averager2d_real
     ! Two dimensional (i-j) data averager.  Takes as input 2D
     ! geospatial fields and generates output fields where each point
     ! combines data from points within X meters.
     real :: radius=0
     integer :: imin=0,imax=0,jmin=0,jmax=0
     class(projection), pointer :: proj=>NULL()
  ...
  type, extends(interpolator) :: bilinear_real
     ...
     class(projection), pointer :: psrc=>NULL(),ptgt=>NULL()
     class(decomp), pointer :: dsrc=>NULL(),dtgt=>NULL()
     logical :: prepped

     real, pointer :: tgtlat(:,:,:)=>NULL(), tgtlon(:,:,:)=>NULL(), &
          srcx(:,:,:)=>NULL(), srcy(:,:,:)=>NULL()
     integer, pointer :: inear(:,:,:,:)=>NULL(),jnear(:,:,:,:)=>NULL(),&
          count(:,:)=>NULL()
     real, pointer :: weight(:,:,:,:)=>NULL(), denom(:,:)=>NULL()

     real, pointer :: cossrc(:,:,:)=>NULL(), sinsrc(:,:,:)=>NULL()
     real, pointer :: costgt(:,:,:)=>NULL(), sintgt(:,:,:)=>NULL()
...
contains ! ----------------------------------------------------
  ...
  subroutine hwind_interpolator(this,tgtu,tgtv,srcu,srcv,op,nomask)
    ! Default implementation of 2D wind vector interpolation.  This
    ! default implementation just interpolates U and V independently
    ! via this%scalar.  Subclasses should replace this routine if
    ! required.
    class(interpolator), intent(inout) :: this
    class(vardata), intent(inout) :: tgtu,tgtv,srcu,srcv
    logical, intent(in) , optional :: nomask
    integer, intent(in), optional :: op
    call this%scalar(tgtu,srcu,op,nomask)
    call this%scalar(tgtv,srcv,op,nomask)
  end subroutine hwind_interpolator

  ! ------------------------------------------------------------

  subroutine clear_averager2d_real(this)
    class(averager2d_real), intent(inout) :: this
    this%radius=0
    this%imin=0
    this%imax=0
    this%jmin=0
    this%jmax=0
    nullify(this%proj,this%dec,this%lat,this%lon)
  end subroutine clear_averager2d_real
  
  subroutine free_averager2d_real(this)
    class(averager2d_real), intent(inout) :: this

    nullify(this%proj,this%dec)
  ...
  end subroutine free_averager2d_real

  subroutine init_averager2d_real(this, radius, proj,dec, imin,imax,jmin,jmax)
    integer, intent(in), optional :: imin,imax,jmin,jmax
    real, intent(in) :: radius
    class(projection), intent(inout) :: proj
    class(decomp), intent(inout) :: dec
    class(averager2d_real), intent(inout) :: this

    call this%clear()
    this%radius=radius
    if(present(imin)) this%imin=imin
    if(present(jmin)) this%jmin=jmin
    if(present(imax)) this%imax=imax
    if(present(jmax)) this%jmax=jmax
  end subroutine init_averager2d_real

  subroutine prep_averager2d_real(this)
    class(averager2d_real), intent(inout) :: this
    class(decomp), pointer :: d
    d=>this%dec
    allocate(this%lat(d%ims:d%ime,d%jms:d%jme,d%kps:d%kps)) 
    allocate(this%lon(d%ims:d%ime,d%jms:d%jme,d%kps:d%kps))

    call this%proj%projinfo( &
         d%ims,d%ime,d%jms,d%jme,d%kms,d%kme, &
         d%ids,d%ide,d%jds,d%jde,d%kds,d%kde, &
         d%ips,d%ipe,d%jps,d%jpe,d%kps,d%kps, &  ! NOTE: end is kps
         lat=this%lat, lon=this%lon)
  end subroutine prep_averager2d_real

  subroutine scalar_averager2d_real(this,tgtvar,srcvar,op,nomask)
    ...
    integer, intent(in), optional :: op
    ! locals
    integer :: i1,j1,i2,j2, imin,jmin,imax,jmax, istart,istop, jstart,jstop,k
    real :: dist,lat1,lon1
    real(kind=8) :: accum
    ...
    if(.not.this%dec%same_dims(tgtvar%dc)) then
       call fail('ERROR: in scalar_averager2d_real, inputs must have same decomposition as averager (tgt mismatch)')
    elseif(.not.this%dec%same_dims(srcvar%dc)) then
       call fail('ERROR: in scalar_averager2d_real, inputs must have same decomposition as averager (src mismatch)')
    endif

    if(op/=SM_MEAN .and. op/=SM_MAX .and. op/=SM_MIN .and. op/=SM_SUM) then
       call fail('ERROR: Invalid value given for scalar_averager2d_real op argument. Only SM_MEAN, SM_MAX, SM_MIN and SM_SUM are allowed.')
    endif

    ! FIXME: should add a projection check here, but that is not
    ! supported at the moment.  Would need to add code to the
    ! projection classes.

    sm=>srcvar%getmask()
    tm=>tgtvar%getmask(alloc=.true.,fill=.false.)
    ...
    if(imin==0) imin=-(d%ide-d%ids+1)
    if(imax==0) imax= (d%ide-d%ids+1)

    if(jmin==0) jmin=-(d%jde-d%jds+1)
    if(jmax==0) jmax= (d%jde-d%jds+1)

    kloop: do k=d%kps,d%kpe
    ! Outer two loops: loop over target grid.
    !$OMP PARALLEL DO PRIVATE(i1,j1,i2,j2, istart,istop,jstart,jstop,&
    !$OMP                     dist,accum,count,lat1,lon1)
    j1loop: do j1=d%jps,d%jpe    ! NOTE: patch dimensions (p)
       i1loop: do i1=d%ips,d%ipe  ! NOTE: patch dimensions (p)
          ! ===============================================================
          ! MPI NOTE: need some communication here due to cross-patch reads
          ! ===============================================================
          istart = max(d%ids,i1-imin) ! NOTE: domain dimensions (d)
          istop  = min(d%ide,i1+imax) !       not patch dimensions (p)
          jstart = max(d%jds,j1-jmin)
          jstop  = min(d%jde,j1+jmax)
          lat1   = this%lat(i1,j1,1)
          lon1   = this%lon(i1,j1,1)
          accum  = 0
          count  = 0
          ! Inner two loops: loop over source gridpoints near target
          ! gridpoint:
          j2loop: do j2=jstart,jstop
             i2loop: do i2=istart,istop
                if(associated(sm)) then
                   if(.not.sm(i2,j2,k)) cycle i2loop ! no data; skip point
                endif
                dist=greatarc(lat1,lon1,this%lat(i2,j2,1),this%lon(i2,j2,1))
                okdist: if(dist<=this%radius) then 
                   whichop: select case(op)
                   case(SM_MEAN)
                      count=count+1
                      accum=accum+sr(i2,j2,k)
                   case(SM_MAX)
                      if(tm(i1,j1,k)) then
                         tr(i1,j1,k)=max(tr(i1,j1,k),sr(i2,j2,k))
                      else
                         tr(i1,j1,k)=sr(i2,j2,k)
                      endif
                   case(SM_MIN)
                      if(tm(i1,j1,k)) then
                         tr(i1,j1,k)=min(tr(i1,j1,k),sr(i2,j2,k))
                      else
                         tr(i1,j1,k)=sr(i2,j2,k)
                      endif
                   case(SM_SUM)
                      if(tm(i1,j1,k)) then
                         tr(i1,j1,k)=tr(i1,j1,k)+sr(i2,j2,k)
                      else
                         tr(i1,j1,k)=sr(i2,j2,k)
                      end if
                   end select whichop
    ...
    enddo j1loop
    enddo kloop
  end subroutine scalar_averager2d_real
...
40              format('ic or den bad at ',I0,',',I0,',',I0,': ic=',I0,' den=',F0.6)
  ...
  subroutine masked_bilinear_real(this,tr,ti,tm,sr,si,sm,op)
    ...
    integer, pointer, intent(inout), dimension(:,:,:) :: ti
    real, pointer, intent(in), dimension(:,:,:) :: sr
    real, pointer, intent(inout), dimension(:,:,:) :: tr
