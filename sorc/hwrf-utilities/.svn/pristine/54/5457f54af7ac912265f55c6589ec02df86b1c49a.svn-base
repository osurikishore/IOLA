     ...
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,DLMD,DPHD,IMAX,JMAX,  
     & use_gfs_nemsio,use_gfs_netcdf) !Zhang
...
C-----------------------------------------------------------------------
     ...
     &,HPTLON1(KB), UPTLON1(KB), VPTLON1(KB)
     &,VPTLAT(KB), VPTLON(KB)
C
      DIMENSION ALL(245)
C
...
C
      ...
      DIMENSION HLON1 (IMJM),VLON1(IMJM)
...
C
     ...
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,DLMD,DPHD) !Zhang
      ...
      write(6,*) 'use_gfs_nemsio====',use_gfs_nemsio
C
...
C
     ...
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX,use_gfs_nemsio,
     & use_gfs_netcdf, alat,alon,ib,jb,nxg,nyg) !Zhang
      print *,'ok after cof2grd'

        write(6,*) 'IMAX, JMAX: ', IMAX,JMAX
        write(6,*) 'KMAX: ', KMAX,JROMB,JCAP
        write(6,*) 'nxg nyg====', nxg, nyg,ib,jb

      if((use_gfs_nemsio .or. use_gfs_netcdf) .and. 
     &               ib+imax .gt. nxg ) then
        if ( HLAT(int(imjm/2)) .gt. 0 ) then
           hlon_min=HLON(imjm-im+1)
        else
           hlon_min=HLON(1)
        endif

        do i=1,IMJM
          if(HLON(i) .gt. hlon_min) then
            HLON1(i)=-360.+HLON(i)
          else
            HLON1(i)=HLON(i)
          endif
          if(VLON(i) .gt. hlon_min) then
            VLON1(i)=-360.+VLON(i)
          else
            VLON1(i)=VLON(i)
          endif
        enddo
        do i=1,NHPT
          if(HPTLON(i) .gt. hlon_min) then
            HPTLON1(i)=-360.+HPTLON(i)
          else
            HPTLON1(i)=HPTLON(i)
          endif
        enddo
        do i=1,NUPT
          if(UPTLON(i) .gt. hlon_min) then
            UPTLON1(i)=-360.+UPTLON(i)
          else
            UPTLON1(i)=UPTLON(i)
          endif
        enddo
        do i=1,NVPT
          if(VPTLON(i) .gt. hlon_min) then
            VPTLON1(i)=-360.+VPTLON(i)
          else
            VPTLON1(i)=VPTLON(i)
          endif
        enddo
...
!zhang test
      DO i = 1,IMJM
      WRITE(39,877) i,HLAT(i),HLON1(i)
877   FORMAT(1X,I6,1X,2(F10.4,1X))
      ENDDO

      if ( .not. use_gfs_nemsio .and. .not. use_gfs_netcdf) then
        DO 521 J=1, JMAX
          YJ = J
          DO 520 I=1, IMAX
            XI = I
...
C  surface pressure at mass points 
     ...
     &            NHPT,HPTLON1,HPTLAT,ALAT,ALON,IFIRST,
     &  WIJ,WIPJ,WIJP,WIPJP,
     &  KIJ,KIPJ,KIJP,KIPJP,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
...
C  surface pressure at u-comp wind points 
     ...
     &            NUPT,UPTLON1,UPTLAT,ALAT,ALON,IFIRST,
     &  WIJ,WIPJ,WIJP,WIPJP,
     &  KIJ,KIPJ,KIJP,KIPJP,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
...
C  surface pressure at v-comp wind points 
     ...
     &            NVPT,VPTLON1,VPTLAT,ALAT,ALON,IFIRST,
     &  WIJ,WIPJ,WIJP,WIPJP,
     &  KIJ,KIPJ,KIJP,KIPJP,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
...
C
      IFIRST=0
      CALL LL2PTS_INIT(PS(1,1,1),PSINIT,IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     ...
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      ...
      CALL LL2PTS_INIT(PS(1,1,2),ZSINIT,IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      ...
      write(6,*) (alat(i,jmax),i=1,imax)
      write(6,*) (alon(i,jmax),i=1,imax)
      write(6,*) (TGRID(i,jmax,1),i=1,imax)
C$OMP PARALLEL DO PRIVATE(K)
      DO K = 1,KMAX
      CALL LL2PTS_INIT(TGRID(1,1,K),TINIT(1,K),IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      CALL LL2PTS_INIT(QGRID(1,1,K),QINIT(1,K),IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      CALL LL2PTS_INIT(CWMGRID(1,1,K),CWMINIT(1,K),IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      CALL LL2PTS_INIT(PRESGRID(1,1,K),PMID(1,K),IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
...
C$OMP END PARALLEL DO
      ...
      write(6,*) (hlat(i),i=imjm-2*im+1,imjm)
      write(6,*) (hlon(i),i=imjm-2*im+1,imjm)
      write(6,*) (TINIT(i,1),i=imjm-2*im+1,imjm)

      IFIRST=0
C$OMP PARALLEL DO PRIVATE(K)
      DO K=1,KMAX+1
      CALL LL2pts_INIT(PINTGRID(1,1,K),PINT(1,K),IMJM,HLON1,HLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
...
C$OMP PARALLEL DO PRIVATE(K)
      DO K = 1,KMAX
      CALL LL2PTS_INIT(UGRID(1,1,K),UINIT(1,K),IMJM,VLON1,VLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
     & KNW,KSW,KSE,KSL,KSLM1,LMM1,LMP1,IMT,JMT,IMAX,JMAX) !Zhang
      CALL LL2PTS_INIT(VGRID(1,1,K),VINIT(1,K),IMJM,VLON1,VLAT,
     1    ALAT,ALON,IFIRST,
     &  WIJ_init,WIPJ_init,WIJP_init,WIPJP_init,
     &  KIJ_init,KIPJ_init,KIJP_init,KIPJP_init,
     & IM,JM,LM,KB,KBETA,IMJM,JMP1,IMM1,IMJMM1,KHL00,KHH00,KNE,
