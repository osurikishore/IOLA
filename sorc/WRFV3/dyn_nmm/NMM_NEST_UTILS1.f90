...
SUBROUTINE med_nest_egrid_configure ( parent , nest )
 ...
 INTEGER :: comzilla
    ...
    IF(MOD(parent%ed32,2) .NE. 0)THEN
     CALL wrf_error_fatal3("<stdin>",49,&
...
"NESTED DOMAIN: JMAX IS EVEN, INCREASE e_sn IN THE namelist.input BY 1")
  ...
  IF ( parent%active_this_task .OR. nest%active_this_task ) THEN
    ...
    if ( .not.nest%analysis ) then
      CALL BYTE_BCAST(  grid%sldpth , size( grid%sldpth )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%dzsoil , size( grid%dzsoil )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%rtdpth , size( grid%rtdpth )*4, comzilla ) 
    endif

     IF ( parent%active_this_task ) THEN
       CALL nl_get_mminlu ( 1, char_junk )
     ENDIF
     CALL wrf_dm_bcast_string_comm( char_junk, LEN(char_junk),comzilla) 
     ...
     ENDIF
      ...
      CALL BYTE_BCAST( grid%cen_lat, 4, comzilla )  
      CALL BYTE_BCAST( grid%cen_lon, 4, comzilla )  
      CALL nl_set_cen_lat ( nest%id , nest%cen_lat)  
      CALL nl_set_cen_lon ( nest%id , nest%cen_lon)  
      ...
      IF ( .NOT. parent%active_this_task ) THEN
        CALL nl_set_cen_lat ( parent%id , nest%cen_lat)  
        CALL nl_set_cen_lon ( parent%id , nest%cen_lon)  
      ENDIF
      ...
      CALL BYTE_BCAST( grid%truelat1, 4, comzilla )  
      CALL BYTE_BCAST( grid%truelat2, 4, comzilla )  
      CALL nl_set_truelat1(nest%id, nest%truelat1)
      CALL nl_set_truelat2(nest%id, nest%truelat2)

      CALL BYTE_BCAST( grid%stand_lon, 4, comzilla )  
      CALL nl_set_stand_lon(nest%id, nest%stand_lon)

      CALL BYTE_BCAST( grid%map_proj, 4, comzilla )  
      CALL BYTE_BCAST( grid%iswater, 4, comzilla )  
      CALL nl_set_map_proj(nest%id, nest%map_proj)
      CALL nl_set_iswater(nest%id, nest%iswater)

      CALL BYTE_BCAST(  grid%deta , size( grid%deta )*4, comzilla )  
      CALL BYTE_BCAST(  grid%aeta , size( grid%aeta )*4, comzilla )  
      CALL BYTE_BCAST(  grid%etax , size( grid%etax )*4, comzilla )  
      CALL BYTE_BCAST(  grid%dfl , size( grid%dfl )*4, comzilla )   
      CALL BYTE_BCAST(  grid%deta1 , size( grid%deta1 )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%aeta1 , size( grid%aeta1 )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%eta1 , size( grid%eta1 )*4, comzilla )  
      CALL BYTE_BCAST(  grid%deta2 , size( grid%deta2 )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%aeta2 , size( grid%aeta2 )*4, comzilla ) 
      CALL BYTE_BCAST(  grid%eta2 , size( grid%eta2 )*4, comzilla )  
      CALL BYTE_BCAST( grid%pdtop, 4, comzilla ) 
      CALL BYTE_BCAST( grid%pt, 4, comzilla )    
      CALL BYTE_BCAST( grid%dfrlg, 4, comzilla ) 
      CALL BYTE_BCAST( grid%num_soil_layers, 4, comzilla ) 
      CALL BYTE_BCAST( grid%num_moves, 4, comzilla )       
...
SUBROUTINE EARTH_LATLON_r8 ( HLAT,HLON,VLAT,VLON,     & 
                            DLMD1,DPHD1,WBD1,SBD1,   & 
                            CENTRAL_LAT,CENTRAL_LON, & 
                            IDS,IDE,JDS,JDE,KDS,KDE, &
                            IMS,IME,JMS,JME,KMS,KME, &
    ...
    ELSE
      ...
      IF(DSLOPE.LE.DSLP0.AND.DSLOPE.GE.-DSLP0)THEN
        IF(TLON.GT.TLONHC)THEN
          IF(TLONHC.GE.-WB-DLM)CALL wrf_error_fatal3("<stdin>",1253,&
"1H:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONHX(I,J)=TLONHC
        ELSE
          IF(TLONHC.LE.WB+DLM)CALL wrf_error_fatal3("<stdin>",1261,&
"2H:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
      ...
      ELSEIF(DSLOPE.GT.DSLP0)THEN
        IF(TLON.GT.TLONHC)THEN
          IF(TLATHC.GE.-SB-DPH)CALL wrf_error_fatal3("<stdin>",1273,&
"3H:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONHX(I,J)=TLONHC-DLM
        ELSE
          IF(TLATHC.LE.SB+DPH)CALL wrf_error_fatal3("<stdin>",1281,&
"4H:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
      ...
      ELSEIF(DSLOPE.LT.-DSLP0)THEN
        IF(TLON.GT.TLONHC)THEN
          IF(TLATHC.LE.SB+DPH)CALL wrf_error_fatal3("<stdin>",1293,&
"5H:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONHX(I,J)=TLONHC-DLM
        ELSE
          IF(TLATHC.GE.-SB-DPH)CALL wrf_error_fatal3("<stdin>",1301,&
    ...
    ELSE
      ...
      IF(DSLOPE.LE.DSLP0.AND.DSLOPE.GE.-DSLP0)THEN
        IF(TLON.GT.TLONVC)THEN
          IF(TLONVC.GE.-WB-DLM)CALL wrf_error_fatal3("<stdin>",1720,&
"1V:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONVX(I,J)=TLONVC
        ELSE
          IF(TLONVC.LE.WB+DLM)CALL wrf_error_fatal3("<stdin>",1728,&
"2V:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
      ...
      ELSEIF(DSLOPE.GT.DSLP0)THEN
        IF(TLON.GT.TLONVC)THEN
          IF(TLATVC.GE.-SB-DPH)CALL wrf_error_fatal3("<stdin>",1739,&
"3V:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONVX(I,J)=TLONVC-DLM
        ELSE
          IF(TLATVC.LE.SB+DPH)CALL wrf_error_fatal3("<stdin>",1747,&
"4V:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
      ...
      ELSEIF(DSLOPE.LT.-DSLP0)THEN
        IF(TLON.GT.TLONVC)THEN
          IF(TLATVC.LE.SB+DPH)CALL wrf_error_fatal3("<stdin>",1758,&
"5V:NESTED DOMAIN TOO CLOSE TO THE BOUNDARY OF PARENT")
          ...
          TLONVX(I,J)=TLONVC-DLM
        ELSE
          IF(TLATVC.GE.-SB-DPH)CALL wrf_error_fatal3("<stdin>",1766,&
...
SUBROUTINE BOUNDS_CHECK( IIH,JJH,IIV,JJV,          &
 ...
 CHARACTER(LEN=255)                 :: message
  ...
  IF(IPOS .LE. SHW)CALL wrf_error_fatal3("<stdin>",2706,&
'NESTED DOMAIN TOO CLOSE TO PARENTs X-BOUNDARY')
  IF(JPOS .LE. SHW)CALL wrf_error_fatal3("<stdin>",2708,&
'NESTED DOMAIN TOO CLOSE TO PARENTs Y-BOUNDARY')

  DO J = JTS, MIN(JTE,JDE-1)
   DO I = ITS, MIN(ITE,IDE-1)
      IF(IIH(I,J) .EQ. 0)CALL wrf_error_fatal3("<stdin>",2713,&
'IIH=0: SOMETHING IS WRONG')
      IF(JJH(I,J) .EQ. 0)CALL wrf_error_fatal3("<stdin>",2715,&
...
CONTAINS
  SUBROUTINE CLIENT(IDS,IDE,JDS,JDE)
    ...
    REAL, DIMENSION(1,1) :: avc_nest,lnd_nest,lu_index_nest
     ...
     call wrf_global_to_patch_real(lnd_nest,nest%hres_lnd,nest%domdesc,'z','xy', &
                                   ...
                                   ims,   ime,   jms,   jme,   1, 1, &
                                   its,   ite,   jts,   jte,   1, 1)
     call wrf_global_to_patch_real(lu_index_nest,nest%hres_lu_index,nest%domdesc,'z','xy', &
                                   ids,   ide-1, jds,   jde-1, 1, 1, &
                                   ims,   ime,   jms,   jme,   1, 1, &
                                   its,   ite,   jts,   jte,   1, 1)
  ...
  SUBROUTINE MASTER(IDS,IDE,JDS,JDE)
    IMPLICIT NONE
    integer, intent(in) :: IDS,IDE,JDS,JDE
    REAL, DIMENSION(IDS:IDE,JDS:JDE)     :: avc_nest, lnd_nest, lu_index_nest
    ...
    DO j=jds,jde
       DO i=ids,ide
          ...
          lu_index_nest(i,j) = tr%lu_index(i_start+i-1 + mod(j+1,2)*i_add, j_start+j-1)
       END DO
    END DO
