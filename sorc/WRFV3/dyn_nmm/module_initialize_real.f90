  ...
  CALL process_soil_real ( adum2d, grid%tg , &
     ...
     num_sw_levels_alloc )
       ...
       (/0.045, 0.057, 0.065, 0.067, 0.034, 0.078, 0.10,     &
        ...
        if ( grid%nmm_tsk(i,j) .lt. 200. .or. &
             grid%nmm_tsk(i,j) .gt. 350. .or. &
             grid%sst(i,j) .lt. 200. .or.     &
             grid%sst(i,j) .gt. 350. ) then
          write(message,*) 'nmm_tsk, sst trouble at : ', i,j
          call wrf_message(message)
          write(message,*) 'sm, nmm_tsk,sst ', grid%sm(i,j),grid%nmm_tsk(i,j),grid%sst(i,j)
          call wrf_message(message)
        endif
 ...
 174      grid%em(JA)=EMJ(J)
  ...
  real function greatarc(lat1,lon1,lat2,lon2)
   ...
   SUBROUTINE make_coupler_fort65(grid,&
     ...
     REAL(KIND=KNUM), DIMENSION(:,:),    ALLOCATABLE :: NHLAT,NHLON,NVLAT,NVLON
     REAL :: NHLAT_temp,NHLON_temp
     REAL, DIMENSION(:,:),    ALLOCATABLE :: HRES_SM, &
...
message)
     ...
     if(.not.wrf_dm_on_monitor()) then
        ...
        call wrf_debug(100,'Working on fort.65 stuff.')
     endif
     ...
     CALL EARTH_LATLON_r8 (  NHLAT,NHLON,NVLAT,NVLON,  & 
                             NDLMD,NDPHD,NWBD,NSBD,    & 
                             tph0d,tlm0d,              & 
...
33            format('ERROR: Invalid HNEAR nest ',I0,',',I0,' parent ',&
           ...
           if(maxweight>0.9999 .and. maxweight<1.03) then
              cenflag(ni,nj)=1
              NHLON_temp=NHLON(ni,nj)
              NHLAT_temp=NHLAT(ni,nj)
              dist=greatarc(bigHLAT(ci,cj,1),bigHLON(ci,cj,1),&
                   NHLAT_temp,NHLON_temp)
              if(dist>100) then
                 bigcount=bigcount+1
              endif
...
message)
     ...
     input_fname='geo_nmm_nest.l01.nc'
     inquire(file=trim(input_fname),exist=file_exists)
     if (file_exists) then
        write(message,*) 'File ',trim(input_fname),' exists. Read in high-resolution sea land masks.'
        call wrf_message(message)
        nullify(tr)
        level=1
        input_type=2
