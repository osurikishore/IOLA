module module_TERRAIN
  ...
  type nmm_terrain
     ...
     real, pointer, dimension(:,:) :: avc,lnd,lah,loh,lu_index
     logical :: initialized
...
3304   format("INVALID NESTING LEVEL ",I0,": only ",I0," through ",I0," are allowed.")
       ...
       call wrf_error_fatal3("<stdin>",26,&
message)
    endif
...
message)
  ...
  subroutine read_terrain(tr,input_type,io_form)
    ...
    input_types: if (input_type == 1) then
       ...
       ALLOCATE (tr%lu_index(tr%nx,tr%ny))
       ...
       CALL get_wrfsi_static_2d(nestpath, 'lu_index', tr%lu_index)
       ...
       read_loop: do n=1,24
          ...
          if (cname(1:10) == "XLAT_M    ") then
             ...
             call wrf_debug(10,'tr%avc.')
          else if (cname(1:10) == "LU_INDEX  ") then
             call wrf_debug(10,'tr%lu_index...')
             ALLOCATE (tr%lu_index(tr%nx,tr%ny))
             call wrf_debug(10,'allocated...')
             do j=1,tr%ny
                do i=1,tr%nx
                   tr%lu_index(i,j) = real_domain(i,j,1)
                end do
             end do
             call wrf_debug(10,'tr%lu_index')
       ...
       if(.not. associated(tr%lu_index))    call readfail(tr,input_fname,'lu_index')
       
    else
       CALL wrf_error_fatal3("<stdin>",262,&
