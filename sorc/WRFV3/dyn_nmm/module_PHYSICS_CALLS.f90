      ...
      USE MODULE_STOCH, ONLY : perturb_mp_tend  
     ...
     &                    ,SWNIRDIR, SWNIRDIF &                            
     &                    ,kte_stoch,kme_stoch                          & 
     &                    ,isppt_rad,pattern_isppt_rad                  & 
     &                    ,stddev_cutoff_isppt_rad,gridpt_stddev_isppt_rad & 
   ...
   REAL, DIMENSION( ims:ime, jms:jme ),                           &
      ...
      REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME),INTENT(OUT):: hrlwpd,hrswpd
    ...
    INTEGER,INTENT(IN)  :: isppt_rad
    REAL                :: thresh1
    INTEGER             :: kh
    REAL,INTENT(IN)     :: stddev_cutoff_isppt_rad,gridpt_stddev_isppt_rad
    REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME_STOCH),INTENT(INOUT) :: pattern_isppt_rad 
...
&rid%swddir,SWDDNI=grid%swddni,SWDDIF=grid%swddif,SWDDIRC=grid%swddirc,SWDDNIC=grid%swddnic,Gx=grid%Gx,Bx=grid%Bx,gg=grid%gg,bb=gri&
&d%bb,swdown_ref=grid%swdown_ref,swddir_ref=grid%swddir_ref,coszen_ref=grid%coszen_ref,coszen=grid%coszen,hrang=grid%hrang,ht=ht,ae&
&r_type=config_flags%aer_type,aer_aod550_opt=config_flags%aer_aod550_opt,aer_aod550_val=config_flags%aer_aod550_val,aer_angexp_opt=&
&config_flags%aer_angexp_opt,aer_angexp_val=config_flags%aer_angexp_val,aer_ssa_opt=config_flags%aer_ssa_opt,aer_ssa_val=config_fla&
&gs%aer_ssa_val,aer_asy_opt=config_flags%aer_asy_opt,aer_asy_val=config_flags%aer_asy_val,RTHRATENLW=THRATENLW,RTHRATENSW=THRATENSW&
&,RTHRATEN=RTHRATEN,HRSWPD=HRSWPD,HRLWPD=HRLWPD,CEN_LAT=grid%cen_lat,GLW=TOTLWDN,GSW=SWNETDN,SWDOWN=TOTSWDN,XLAT=grid%HLAT,XLONG=gr&
&id%HLON,ALBEDO=ALBEDO,EMISS=EPSR,XICE=SICE,XLAND=XLAND,Z=Z,TSK=TSFC,N_AEROSOLC=NUM_AEROSOLC,PAERLEV=GRID%PAERLEV,ID=grid%id,CAM_AB&
&S_DIM1=GRID%CAM_ABS_DIM1,CAM_ABS_DIM2=GRID%CAM_ABS_DIM2,CAM_ABS_FREQ_S=GRID%CAM_ABS_FREQ_S,ALEVSIZ=grid%alevsiz,no_src_types=grid%&
&no_src_types,LEVSIZ=LEVSIZ,N_OZMIXM=NUM_OZMIXM,OZMIXM=OZMIXM,PIN=PIN,HTOP=HTOP,HBOT=HBOT,CUPPT=CUPPTR,HTOPR=HTOPR,HBOTR=HBOTR,VEGF&
&RA=VEGFRC,SNOW=SNOW,RHO=RR,P8W=P8W,P=P_PHY,PI=PI_PHY,DZ8W=DZ,T=T_PHY,T8W=T8W,GMT=GMT,JULDAY=JULDAY,JULYR=JULYR,NPHS=NPHS,JULIAN=JU&
&LIAN,XTIME=XTIME,YR=JULYR,LW_PHYSICS=CONFIG_FLAGS%RA_LW_PHYSICS,SW_PHYSICS=CONFIG_FLAGS%RA_SW_PHYSICS,RADT=RADT,RA_CALL_OFFSET=GRI&
&D%RA_CALL_OFFSET,STEPRA=NRAD,ICLOUD=config_flags%ICLOUD,cldovrlp=config_flags%cldovrlp,idcor=config_flags%idcor,WARM_RAIN=WARM_RAI&
&N,SWDOWNC=TOTSWDNC,CLDFRA=CLFR,SWUPT=SWUPT,SWUPTC=SWUPTC,SWDNT=SWDNT,SWDNTC=SWDNTC,SWUPB=SWUPB,SWUPBC=SWUPBC,SWDNB=SWDNB,SWDNBC=SW&
&DNBC,LWUPT=LWUPT,LWUPTC=LWUPTC,LWDNT=LWDNT,LWDNTC=LWDNTC,LWUPB=LWUPB,LWUPBC=LWUPBC,LWDNB=LWDNB,LWDNBC=LWDNBC,ACSWUPT=ACSWUPT,ACSWU&
&PTC=ACSWUPTC,ACSWDNT=ACSWDNT,ACSWDNTC=ACSWDNTC,ACSWUPB=ACSWUPB,ACSWUPBC=ACSWUPBC,ACSWDNB=ACSWDNB,ACSWDNBC=ACSWDNBC,ACLWUPT=ACLWUPT&
&,ACLWUPTC=ACLWUPTC,ACLWDNT=ACLWDNT,ACLWDNTC=ACLWDNTC,ACLWUPB=ACLWUPB,ACLWUPBC=ACLWUPBC,ACLWDNB=ACLWDNB,ACLWDNBC=ACLWDNBC,SWVISDIR=&
&swvisdir,SWVISDIF=swvisdif,SWNIRDIR=swnirdir,SWNIRDIF=swnirdif,re_cloud=grid%re_cloud,re_ice=grid%re_ice,re_snow=grid%re_snow,has_&
&reqc=has_reqc,has_reqi=has_reqi,has_reqs=has_reqs,RSWTOA=RSWTOA,RLWTOA=RLWTOA,CZMEAN=CZMEAN,CFRACL=CFRACL,CFRACM=CFRACM,CFRACH=CFR&
&ACH,ACFRST=ACFRST,NCFRST=NCFRST,ACFRCV=ACFRCV,NCFRCV=NCFRCV,F_ICE_PHY=F_ICE,F_RAIN_PHY=F_RAIN,LWCF=LWCF,SWCF=SWCF,O3INPUT=config_f&
&lags%O3INPUT,AER_OPT=config_flags%AER_OPT,O3RAD=grid%o3rad,SF_SURFACE_PHYSICS=CONFIG_FLAGS%SF_SURFACE_PHYSICS,QV=MOIST_TRANS(IMS,K&
&MS,JMS,P_QV),F_QV=F_QV,QC=MOIST_TRANS(IMS,KMS,JMS,P_QC),F_QC=F_QC,QR=MOIST_TRANS(IMS,KMS,JMS,P_QR),F_QR=F_QR,QI=MOIST_TRANS(IMS,KM&
&S,JMS,P_QI),F_QI=F_QI,QS=MOIST_TRANS(IMS,KMS,JMS,P_QS),F_QS=F_QS,QG=MOIST_TRANS(IMS,KMS,JMS,P_QG),F_QG=F_QG,IS_CAMMGMP_USED=IS_CAM&
&MGMP_USED,EXPLICIT_CONVECTION=config_flags%cu_physics==0,AERCU_OPT=config_flags%aercu_opt,CU_PHYSICS=config_flags%cu_physics,MP_PH&
&YSICS=CONFIG_FLAGS%MP_PHYSICS,CALC_CLEAN_ATM_DIAG=CONFIG_FLAGS%CALC_CLEAN_ATM_DIAG)

    IF (config_flags%isppt_on==1) THEN 
      thresh1=stddev_cutoff_isppt_rad*gridpt_stddev_isppt_rad
!$omp parallel do                                             &
!$omp& private(i,j,k)
      DO j = jts,jte
      DO k = kts,MIN(kte-1,kte_stoch-1)
      DO i = its,ite
        pattern_isppt_rad(i,j,k)=MAX(MIN(pattern_isppt_rad(i,j,k),thresh1),-1.*thresh1)
      ENDDO
      ENDDO
      ENDDO
     ENDIF
     ...
     IF (config_flags%isppt_rad .GT. 0 ) THEN  
!$omp parallel do                                             &
!$omp& private(i,j,k,kh)
       DO j = max(jds+( 2 ),jts-( 0  )),min(jde-( 2 ),jte+( 0  )) 
       DO k = kts , MIN(kte,kde-1)
         kh = MIN( k, kte_stoch - 1 )
       DO i = max(ids+( 1 ),its-( 0  )),min(ide-( 1 ),ite+( 0  )) 
         RTHRATEN(i,k,j) = RTHRATEN(i,k,j)+pattern_isppt_rad(i,j,kh)*RTHRATEN(i,k,j)
       ENDDO
       ENDDO
       ENDDO
     ENDIF
...
!$omp& private(i,j,k)
     ...
     &                ,hpbl_hold                                        & 
     &                ,kte_stoch,kme_stoch                              & 
     &                ,isppt_pbl,pattern_isppt_pbl                      & 
     &                ,stddev_cutoff_isppt_pbl,gridpt_stddev_isppt_pbl  & 
     &                ,IDS,IDE,JDS,JDE,KDS,KDE                          &
     &                ,IMS,IME,JMS,JME,KMS,KME                          &
     ...
     &                                              ,UGWDsfc,VGWDsfc      
      ...
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: CDX 
      ...
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: taux, tauy
     ...
     &                                          ,U_PHY,V_PHY,Z
      ...
      REAL,DIMENSION(:,:,:,:),ALLOCATABLE :: MOIST_TRANS, SCALAR_TRANS 
    ...
    INTEGER,INTENT(IN)  :: kte_stoch,kme_stoch
    INTEGER,INTENT(IN)  :: isppt_pbl
    INTEGER             :: kh
    REAL                :: thresh1
    REAL,INTENT(IN)     :: stddev_cutoff_isppt_pbl,gridpt_stddev_isppt_pbl
    REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME_STOCH),INTENT(INOUT) :: pattern_isppt_pbl
...
"NMM must use SEAICE_ALBEDO_OPT = 0")
      ...
      DX=grid%dlmd*0.01745329*6371200.
      DY=DX
...
!$omp& private(factrl,i,j,llij,tlmh)
      ...
      DO I=max(ids+( 0 ),its-( 0  )),min(ide-( 0 ),ite+( 0  ))
        ...
        SST(I,J)=SST(I,J)*SM(I,J)+TSFC(I,J)*(1.-SM(I,J)) 
...
&C=QS,QSHLTR=QSHLTR,QZ0=QZ0,ICOEF_SF=CONFIG_FLAGS%ICOEF_SF,LCURR_SF=CONFIG_FLAGS%LCURR_SF,ZKMAX=ZKMAX,RIBN=RIBN,CHARN=CHARN,MSANG=M&
&SANG,SCURX=SCURX,SCURY=SCURY,IWAVECPL=CONFIG_FLAGS%IWAVECPL,pert_Cd=config_flags%pert_Cd,ens_random_seed=config_flags%ens_random_s&
&eed,ens_Cdamp=config_flags%ens_Cdamp,cd_out=grid%cd_out,ch_out=grid%ch_out,cdx=grid%cdx,RAINCV=RAIN,RHO=RR,SFCEVP=SFCEVPX,SFCEXC=S&
&FCEXC,SFCRUNOFF=SSROFF,SMOIS=SMC,SMSTAV=SMSTAV,SMSTOT=SMSTOT,SNOALB=MXSNAL,SNOW=SNOW,SNOWC=SNOWC,SNOWH=SNOWH,STEPBL=NPHS,SMCREL=SM&
&CREL,SST=SST,SST_UPDATE=SST_UPDATE,MAX_EDOM=-1,TH10=TH10,TH2=TH2X,T2=T2,THZ0=THZ0,TH_PHY=TH_PHY,TMN=TG,TSHLTR=TSHLTR,TSK=TSFC,TSLB&
&=STC,T_PHY=T_PHY,U10=U10,UDRUNOFF=BGROFF,UST=USTAR,UZ0=UZ0H,U_FRAME=U_FRAME,U_PHY=U_PHY,V10=V10,VEGFRA=VGFRCK,UOCE=UOCE,VOCE=VOCE,&
&VZ0=VZ0H,V_FRAME=V_FRAME,V_PHY=V_PHY,WARM_RAIN=WARM_RAIN,WSPD=WSPD,XICE=SICE,XICEM=SICE,ALBSI=albsi,ICEDEPTH=icedepth,SNOWSI=snows&
&i,ISICE=GRID%LANDUSE_ISICE,ISWATER=GRID%ISWATER,XLAND=XLAND,Z=Z,ZNT=Z0,MZNT=MZ0,ZS=SLDPTH,CT=CT,TKE_PBL=TKE,SFENTH=SFENTH,ALBBCK=A&
&LBASE,LH=ELFLX,SH2O=SH2O,SHDMAX=SHDMAX,SHDMIN=SHDMIN,Z0=Z0BASE,FLQC=FLQC,FLHC=FLHC,FM=FM,FHH=FH,ZOL=ZOL,PSFC=PSFC_OUT,EMISS=EPSR,E&
&MBCK=EMBCK,SF_SFCLAY_PHYSICS=CONFIG_FLAGS%SF_SFCLAY_PHYSICS,SF_SURFACE_PHYSICS=CONFIG_FLAGS%SF_SURFACE_PHYSICS,RA_LW_PHYSICS=CONFI&
&G_FLAGS%RA_LW_PHYSICS,RA_SW_PHYSICS=CONFIG_FLAGS%RA_SW_PHYSICS,LAI=GRID%LAI,IZ0TLND=CONFIG_FLAGS%IZ0TLND,SF_URBAN_PHYSICS=SF_URBAN&
&_PHYSICS,ISFTCFLX=config_flags%isftcflx,NUM_URBAN_LAYERS=NUM_URBAN_LAYERS,NUM_URBAN_HI=config_flags%num_urban_hi,IDS=IDS,IDE=IDE,J&
&DS=JDS,JDE=JDE,KDS=KDS,KDE=KDE,IMS=IMS,IME=IME,JMS=JMS,JME=JME,KMS=KMS,KME=KME,IPS=ips,IPE=ipe,JPS=jps,JPE=jpe,KPS=kps,KPE=kpe,I_S&
&TART=GRID%I_START,I_END=GRID%I_END,J_START=GRID%J_START,J_END=GRID%J_END,KTS=KTS,KTE=KTE,NUM_TILES=GRID%NUM_TILES,QV_CURR=MOIST_TR&
&ANS(IMS,KMS,JMS,P_QV),F_QV=F_QV,QC_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QC),F_QC=F_QC,QR_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QR),F_QR=F_QR,QI&
&_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QI),F_QI=F_QI,QS_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QS),F_QS=F_QS,QG_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QG&
&),F_QG=F_QG,RAINBL=RAINBL,LAGDAY=1,QSG=QSG,QVG=QVG,QCG=QCG,SOILT1=SOILT1,TSNAV=TSNAV,SMFR3D=SMFR3D,KEEPFR3DFLAG=KEEPFR3DFLAG,POTEV&
&P=POTEVP,SNOPCX=SNOPCX,SOILTB=SOILTB,SR=SR,DEW=DEW,ACRUNOFF=grid%ACRUNOFF,MOSAIC_LU=CONFIG_FLAGS%MOSAIC_LU,MOSAIC_SOIL=CONFIG_FLAG&
&S%MOSAIC_SOIL,LANDUSEF=grid%landusef,SOILCTOP=grid%soilctop,rhosnf=grid%rhosnf,precipfr=grid%precipfr,snowfallac=grid%snowfallac,N&
&UM_ROOF_LAYERS=NUM_ROOF_LAYERS,NUM_WALL_LAYERS=NUM_WALL_LAYERS,NUM_ROAD_LAYERS=NUM_ROAD_LAYERS,REGIME=REGIME,NLCAT=grid%num_land_c&
&at,NSCAT=grid%num_soil_cat,ISURBAN=isurban,MMINLU=TRIM(mminlu),SNOTIME=grid%SNOTIME,RDLAI2D=config_flags%rdlai2d,usemonalb=config_&
&flags%usemonalb,NOAHRES=grid%noahres,opt_thcnd=config_flags%opt_thcnd,ua_phys=config_flags%ua_phys,flx4=grid%flx4,fvb=grid%fvb,fbu&
&r=grid%fbur,fgsn=grid%fgsn,okms=1,okme=2,YR=yr,idveg=config_flags%dveg,iopt_crs=config_flags%opt_crs,iopt_btr=config_flags%opt_btr&
&,iopt_run=config_flags%opt_run,iopt_sfc=config_flags%opt_sfc,iopt_frz=config_flags%opt_frz,iopt_inf=config_flags%opt_inf,iopt_rad=&
&config_flags%opt_rad,iopt_alb=config_flags%opt_alb,iopt_snf=config_flags%opt_snf,iopt_tbot=config_flags%opt_tbot,iopt_stc=config_f&
&lags%opt_stc,iopt_gla=config_flags%opt_gla,iopt_rsf=config_flags%opt_rsf,iopt_soil=config_flags%opt_soil,iopt_pedo=config_flags%op&
&t_pedo,iopt_crop=config_flags%opt_crop,isnowxy=grid%isnowxy,tvxy=grid%tvxy,tgxy=grid%tgxy,canicexy=grid%canicexy,canliqxy=grid%can&
&liqxy,eahxy=grid%eahxy,tahxy=grid%tahxy,cmxy=grid%cmxy,chxy=grid%chxy,fwetxy=grid%fwetxy,sneqvoxy=grid%sneqvoxy,alboldxy=grid%albo&
&ldxy,qsnowxy=grid%qsnowxy,wslakexy=grid%wslakexy,zwtxy=grid%zwtxy,waxy=grid%waxy,wtxy=grid%wtxy,tsnoxy=grid%tsnoxy,zsnsoxy=grid%zs&
&nsoxy,snicexy=grid%snicexy,snliqxy=grid%snliqxy,lfmassxy=grid%lfmassxy,rtmassxy=grid%rtmassxy,stmassxy=grid%stmassxy,woodxy=grid%w&
&oodxy,stblcpxy=grid%stblcpxy,fastcpxy=grid%fastcpxy,grainxy=grid%grainxy,gddxy=grid%gddxy,pgsxy=grid%pgsxy,cropcat=grid%cropcat,pl&
&anting=grid%planting,harvest=grid%harvest,season_gdd=grid%season_gdd,soilcomp=grid%soilcomp,soilcl1=grid%soilcl1,soilcl2=grid%soil&
&cl2,soilcl3=grid%soilcl3,soilcl4=grid%soilcl4,xsaixy=grid%xsaixy,taussxy=grid%taussxy,t2mvxy=grid%t2mvxy,t2mbxy=grid%t2mbxy,q2mvxy&
&=grid%q2mvxy,q2mbxy=grid%q2mbxy,tradxy=grid%tradxy,neexy=grid%neexy,gppxy=grid%gppxy,nppxy=grid%nppxy,fvegxy=grid%fvegxy,runsfxy=g&
&rid%runsfxy,runsbxy=grid%runsbxy,ecanxy=grid%ecanxy,edirxy=grid%edirxy,etranxy=grid%etranxy,fsaxy=grid%fsaxy,firaxy=grid%firaxy,ap&
&arxy=grid%aparxy,psnxy=grid%psnxy,savxy=grid%savxy,sagxy=grid%sagxy,rssunxy=grid%rssunxy,rsshaxy=grid%rsshaxy,bgapxy=grid%bgapxy,w&
&gapxy=grid%wgapxy,tgvxy=grid%tgvxy,tgbxy=grid%tgbxy,chvxy=grid%chvxy,chbxy=grid%chbxy,shgxy=grid%shgxy,shcxy=grid%shcxy,shbxy=grid&
&%shbxy,evgxy=grid%evgxy,evbxy=grid%evbxy,ghvxy=grid%ghvxy,ghbxy=grid%ghbxy,irgxy=grid%irgxy,ircxy=grid%ircxy,irbxy=grid%irbxy,trxy&
&=grid%trxy,evcxy=grid%evcxy,chleafxy=grid%chleafxy,chucxy=grid%chucxy,chv2xy=grid%chv2xy,chb2xy=grid%chb2xy,chstarxy=grid%chstarxy&
&,smoiseq=grid%smoiseq,smcwtdxy=grid%smcwtdxy,rechxy=grid%rechxy,deeprechxy=grid%deeprechxy,gecros_state=grid%gecros_state,coszen=g&
&rid%czen,xlat_urb2d=grid%hlat,rs=rs,xlaidyn=xlaidyn,sf_surface_mosaic=config_flags%sf_surface_mosaic,mosaic_cat=config_flags%mosai&
&c_cat,lakedepth2d=grid%lakedepth2d,savedtke12d=grid%savedtke12d,snowdp2d=grid%snowdp2d,h2osno2d=grid%h2osno2d,snl2d=grid%snl2d,t_g&
&rnd2d=grid%t_grnd2d,t_lake3d=grid%t_lake3d,lake_icefrac3d=grid%lake_icefrac3d,z_lake3d=grid%z_lake3d,dz_lake3d=grid%dz_lake3d,t_so&
&isno3d=grid%t_soisno3d,h2osoi_ice3d=grid%h2osoi_ice3d,h2osoi_liq3d=grid%h2osoi_liq3d,h2osoi_vol3d=grid%h2osoi_vol3d,z3d=grid%z3d,d&
&z3d=grid%dz3d,zi3d=grid%zi3d,watsat3d=grid%watsat3d,csol3d=grid%csol3d,tkmg3d=grid%tkmg3d,tkdry3d=grid%tkdry3d,tksatu3d=grid%tksat&
&u3d,LakeModel=grid%sf_lake_physics,lake_min_elev=grid%lake_min_elev,maxpatch=1,inest=1,history_interval=config_flags%history_inter&
&val,fasdas=fasdas,ch=grid%ch,tsq=grid%tsq,qsq=grid%qsq,cov=grid%cov,Sh3d=grid%sh3d,el_pbl=el_myj,bl_mynn_cloudpdf=config_flags%bl_&
&mynn_cloudpdf,icloud_bl=config_flags%icloud_bl,qc_bl=grid%qc_bl,cldfra_bl=grid%cldfra_bl,fgdp=grid%fgdp,dfgdp=grid%dfgdp,vdfg=grid&
&%vdfg,grav_settling=config_flags%grav_settling)
...
&RQVBLTEN,RQCBLTEN=RQCBLTEN,RQIBLTEN=RQIBLTEN,TSK=TSFC,XLAND=XLAND,ZNT=Z0,MSANG=MSANG,SCURX=SCURX,SCURY=SCURY,IWAVECPL=CONFIG_FLAGS&
&%IWAVECPL,LCURR_SF=CONFIG_FLAGS%LCURR_SF,MZNT=MZ0,HPBL_HOLD=HPBL_HOLD,HT=SFCZ,UST=USTAR,MIXHT=MIXHT,PBLH=PBLH,HFX=TWBS,QFX=QWBS,GR&
&DFLX=GRNFLX,U_PHY=U_PHY,V_PHY=V_PHY,TH_PHY=TH_PHY,RHO=RR,P_PHY=P_PHY,PI_PHY=PI_PHY,P8W=P8W,T_PHY=T_PHY,DZ8W=DZ,Z=Z,TKE_PBL=TKE,EL_&
&PBL=EL_MYJ,F=F,EXCH_H=EXCH_H,EXCH_M=EXCH_M,AKHS=AKHS,AKMS=AKMS,FM=FM,FHH=FH,YSU_TOPDOWN_PBLMIX=CONFIG_FLAGS%YSU_TOPDOWN_PBLMIX,ysu&
&_firstorder=config_flags%ysu_firstorder,ysu_brcr_ub=config_flags%ysu_brcr_ub,ysu_cgrad_scalar=config_flags%ysu_cgrad_scalar,ysu_cg&
&rad_mom=config_flags%ysu_cgrad_mom,SHINHONG_TKE_DIAG=CONFIG_FLAGS%SHINHONG_TKE_DIAG,THZ0=THZ0,QZ0=QZ0,UZ0=UZ0H,VZ0=VZ0H,QSFC=QS,LO&
&WLYR=LOWLYR,PSIM=PSIM,PSIH=PSIH,GZ1OZ0=GZ1OZ0,U10=U10,V10=V10,UOCE=UOCE,VOCE=VOCE,T2=T2,WSPD=WSPD,BR=BR,CHKLOWQ=CHKLOWQ,DX=DX,DY=D&
&Y,STEPBL=NPHS,WARM_RAIN=WARM_RAIN,KPBL=KPBL,CT=CT,LH=ELFLX,SNOW=SNOW,XICE=SICE,BL_PBL_PHYSICS=config_flags%bl_pbl_physics,RA_LW_PH&
&YSICS=config_flags%ra_lw_physics,MFSHCONV=config_flags%mfshconv,rc_mf=rc_mf,IDS=IDS,IDE=IDE,JDS=JDS,JDE=JDE,KDS=KDS,KDE=KDE,IMS=IM&
&S,IME=IME,JMS=JMS,JME=JME,KMS=KMS,KME=KME,I_START=GRID%I_START,I_END=GRID%I_END,J_START=GRID%J_START,J_END=GRID%J_END,KTS=KTS,KTE=&
&KTE,NUM_TILES=GRID%NUM_TILES,CTOPO=grid%ctopo,CTOPO2=grid%ctopo2,WINDFARM_OPT=config_flags%windfarm_opt,POWER=grid%POWER,NUM_SCALA&
&R=1,NUM_TRACER=1,DISHEAT=DISHEAT,RTHRATEN=RTHRATEN,GFS_ALPHA=GRID%GFS_ALPHA,HPBL2D=HPBL2D,EVAP2D=EVAP2D,HEAT2D=HEAT2D,RC2D=RC2D,VA&
&R_RIC=VAR_RIC,DKU3D=DKU3D,DKT3D=DKT3D,coef_ric_l=coef_ric_l,coef_ric_s=coef_ric_s,pert_pbl=pert_pbl,ens_random_seed=ens_random_see&
&d,ens_pblamp=ens_pblamp,QV_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QV),F_QV=F_QV,QC_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QC),F_QC=F_QC,QI_CURR=MO&
&IST_TRANS(IMS,KMS,JMS,PQ_I),F_QI=FQ_I,QR_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QR),F_QR=F_QR,QS_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QS),F_QS=F&
&_QS,QG_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QG),F_QG=F_QG,HOL=HOL,sf_sfclay_physics=CONFIG_FLAGS%SF_SFCLAY_PHYSICS,QKE=grid%qke,Sh3d=gri&
&d%sh3d,QKE_ADV=scalar_TRANS(ims,kms,jms,P_qke_adv),bl_mynn_tkeadvect=config_flags%bl_mynn_tkeadvect,tsq=grid%tsq,qsq=grid%qsq,cov=&
...
&tling,vdfg=grid%vdfg,QNI_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QNI),F_QNI=F_QNI,RQNIBLTEN=RQNIBLTEN,IS_CAMMGMP_USED=IS_CAMMGMP_USED,wstar&
&=wstar_ysu,delta=delta_ysu,fasdas=fasdas,sf_urban_physics=CONFIG_FLAGS%SF_URBAN_PHYSICS)
    IF (config_flags%isppt_on==1) THEN 
      thresh1=stddev_cutoff_isppt_pbl*gridpt_stddev_isppt_pbl
!$omp parallel do                                                       &
!$omp& private(i,j,k)
      DO j = jts,jte
      DO k = kts,MIN(kte-1,kte_stoch-1)
      DO i = its,ite
        pattern_isppt_pbl(i,j,k)=MAX(MIN(pattern_isppt_pbl(i,j,k),thresh1),-1.*thresh1)
      ENDDO
      ENDDO
      ENDDO
    ENDIF
    IF (config_flags%bl_pbl_physics .GT. 0) THEN 
    IF (config_flags%isppt_pbl .GT. 0 ) THEN  
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      DO j = max(jds+( 0 ),jts-( 0  )),min(jde-( 0 ),jte+( 0  )) 
      DO k = kts, MIN(kte,kde-1)
        kh = MIN( k, kte_stoch - 1 )
      DO i = max(ids+( 0 ),its-( 0  )),min(ide-( 0 ),ite+( 0  )) 
        DUDT_PHY(i,k,j) = DUDT_PHY(i,k,j)+pattern_isppt_pbl(i,j,kh)*DUDT_PHY(i,k,j)
      ENDDO
      ENDDO
      ENDDO
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      ...
      DO k = kts, MIN(kte,kde-1)
        kh = MIN( k, kte_stoch - 1 )
      DO i = max(ids+( 0 ),its-( 0  )),min(ide-( 0 ),ite+( 0  )) 
        DVDT_PHY(i,k,j) = DVDT_PHY(i,k,j)+pattern_isppt_pbl(i,j,kh)*DVDT_PHY(i,k,j)
      ENDDO
      ENDDO
      ENDDO
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      ...
      DO k = kts , MIN(kte,kde-1)
        kh = MIN( k, kte_stoch - 1 )
      DO i = max(ids+( 0 ),its-( 0  )),min(ide-( 0 ),ite+( 0  )) 
        RTHBLTEN(i,k,j) = RTHBLTEN(i,k,j)+pattern_isppt_pbl(i,j,kh)*RTHBLTEN(i,k,j)
        RQVBLTEN(i,k,j) = RQVBLTEN(i,k,j)+pattern_isppt_pbl(i,j,kh)*RQVBLTEN(i,k,j)
      ENDDO
      ENDDO
      ENDDO
    ENDIF 
...
!$omp& private(i,iend,j,k)
     ...
     &                 ,DTHCUDT,DQVCUDT,DQRCUDT,DQCCUDT,DQICUDT         & 
     &                 ,DQSCUDT                                         & 
     &                 ,kte_stoch,kme_stoch                             & 
     &                 ,isppt_conv,pattern_isppt_conv                   & 
     &                 ,stddev_cutoff_isppt_conv,gridpt_stddev_isppt_conv &
     ...
     &                     ,N_MOIST,NCNVC,NTSD,NRADS,NRADL,N_SCALAR
      ...
      REAL,INTENT(IN) :: DYNMM                           
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: SCALEFUN,SCALEFUN1 
                                                                     
       REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: SIGMU,SIGMU1 
     ...
     &                                                        ,GD_CLOUD2
      REAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) ::    XMB_SHALLOW      
      INTEGER,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: KTOP_DEEP        
      INTEGER,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: K22_SHALLOW   &  
     &                                                   ,KBCON_SHALLOW &  
     &                                                   ,KTOP_SHALLOW     
      INTEGER,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT) :: IERR_SHALLOW     
     ...
     &                                          ,U_PHY,V_PHY,WINT
      ...
      real    :: ens_sasamp         
    INTEGER,INTENT(IN)  :: kte_stoch,kme_stoch
    INTEGER,INTENT(IN)  :: isppt_conv
    INTEGER             :: kh
    REAL                :: thresh1
    REAL,INTENT(IN)     :: stddev_cutoff_isppt_conv,gridpt_stddev_isppt_conv
    REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME_STOCH),INTENT(INOUT) :: pattern_isppt_conv
...
!$omp& private(i,j)
     ...
     &   CONFIG_FLAGS%CU_PHYSICS==SCALESASSCHEME)RETURN        
      IF(MOD(NTSD,NCNVC)/=0.AND.                                      &
     &   CONFIG_FLAGS%CU_PHYSICS==GFSCHEME)RETURN             
...
&RCEQ=QTEN,CLDEFI=CLDEFI,LOWLYR=LOWLYR,XLAND=XLAND,CU_ACT_FLAG=CU_ACT_FLAG,WARM_RAIN=WARM_RAIN,UDR_KF=grid%udr_kf,DDR_KF=grid%ddr_k&
&f,UER_KF=grid%uer_kf,DER_KF=grid%der_kf,TIMEC_KF=grid%timec_kf,KF_EDRATES=config_flags%kf_edrates,HFX=HFX,QFX=QFX,PBLH=PBLH,HPBL_H&
&OLD=HPBL_HOLD,ZNU=ZNU,STEPCU=NSTEP_CNV,GSW=GSW,PERIODIC_X=.FALSE.,PERIODIC_Y=.FALSE.,HTOP=CUTOP,HBOT=CUBOT,KPBL=KPBL,HT=SFCZ,Z=Z,A&
&PR_GR=APR_GR,APR_W=APR_W,APR_MC=APR_MC,APR_ST=APR_ST,APR_AS=APR_AS,APR_CAPMA=APR_CAPMA,APR_CAPME=APR_CAPME,APR_CAPMI=APR_CAPMI,MAS&
&S_FLUX=MASS_FLUX,XF_ENS=XF_ENS,PR_ENS=PR_ENS,ishallow=config_flags%ishallow,clos_choice=config_flags%clos_choice,GD_CLOUD=GD_CLOUD&
&,GD_CLOUD2=GD_CLOUD2,KTOP_DEEP=KTOP_DEEP,k22_shallow=k22_shallow,kbcon_shallow=kbcon_shallow,ktop_shallow=ktop_shallow,xmb_shallow&
&=xmb_shallow,ierr_shallow=ierr_shallow,ENSDIM=ENSDIM,MAXIENS=1,MAXENS=3,MAXENS2=3,MAXENS3=16,RTHCUTEN=RTHCUTEN,RQVCUTEN=RQVCUTEN,R&
&QCCUTEN=RQCCUTEN,RQRCUTEN=RQRCUTEN,RQICUTEN=RQICUTEN,RQSCUTEN=RQSCUTEN,RTHBLTEN=RTHBLTEN,RQVBLTEN=RQVBLTEN,RTHRATEN=RTHRATEN,RUCUT&
&EN=DUCUDT,RVCUTEN=DVCUDT,MOMMIX=MOMMIX,store_rand=store_rand,pert_sas=pert_sas,ens_random_seed=ens_random_seed,ens_sasamp=ens_sasa&
&mp,SHALL=grid%shall,HPBL2D=HPBL2D,EVAP2D=EVAP2D,HEAT2D=HEAT2D,DX2D=DX2D,DYNMM=DYNMM,SCALEFUN=SCALEFUN,SCALEFUN1=SCALEFUN1,SIGMU=SI&
&GMU,SIGMU1=SIGMU1,pgcon=config_flags%sas_pgcon,sas_mass_flux=config_flags%sas_mass_flux,shalconv=config_flags%sas_shal_conv,shal_p&
&gcon=config_flags%sas_shal_pgcon,IS_CAMMGMP_USED=IS_CAMMGMP_USED,CU_PHYSICS=CONFIG_FLAGS%CU_PHYSICS,MP_PHYSICS=CONFIG_FLAGS%MP_PHY&
&SICS,QV_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QV),F_QV=F_QV,QC_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QC),F_QC=F_QC,QR_CURR=MOIST_TRANS(IMS,KMS,J&
&MS,P_QR),F_QR=F_QR,QI_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QI),F_QI=F_QI,QS_CURR=MOIST_TRANS(IMS,KMS,JMS,P_QS),F_QS=F_QS,QG_CURR=MOIST_T&
&RANS(IMS,KMS,JMS,P_QG),F_QG=F_QG)
    IF (config_flags%isppt_on==1) THEN 
      thresh1=stddev_cutoff_isppt_conv*gridpt_stddev_isppt_conv
!$omp parallel do                                                       &
!$omp& private(i,j,k)
      DO j = jts,jte
      DO k = kts,MIN(kte-1,kte_stoch-1)
      DO i = its,ite
        pattern_isppt_conv(i,j,k)=MAX(MIN(pattern_isppt_conv(i,j,k),thresh1),-1.*thresh1)
      ENDDO
      ENDDO
      ENDDO
    ENDIF
    IF (config_flags%cu_physics .GT. 0) THEN 
    IF (config_flags%isppt_conv .GT. 0 ) THEN  
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      DO j = max(jds+( 1 ),jts-( 1  )),min(jde-( 1 ),jte+( 1  )) 
      DO k = kts, MIN(kte,kde-1)
        kh = MIN( k, kte_stoch - 1 )
      DO i = max(ids+( 0 ),its-( 1  )),min(ide-( 0 ),ite+( 1  )) 
        DUCUDT(i,j,k)=DUCUDT(i,j,k)+pattern_isppt_conv(i,j,kh)*DUCUDT(i,j,k)
      ENDDO
      ENDDO
      ENDDO
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      ...
      DO k = kts, MIN(kte,kde-1)
        kh = MIN( k, kte_stoch - 1 )
      DO i = max(ids+( 0 ),its-( 1  )),min(ide-( 0 ),ite+( 1  )) 
        DVCUDT(i,j,k)=DVCUDT(i,j,k)+pattern_isppt_conv(i,j,kh)*DVCUDT(i,j,k)
      ENDDO
      ENDDO
      ENDDO
!$omp parallel do                                                       &
!$omp& private(i,j,k,kh)
      ...
      DO k = kts , min(kte,kde-1)
        kh = min( k, kte_stoch - 1 )
      DO i = max(ids+( 1 ),its-( 0  )),min(ide-( 1 ),ite+( 0  )) 
        RTHCUTEN(i,k,j)=RTHCUTEN(i,k,j)+pattern_isppt_conv(i,j,kh)*RTHCUTEN(i,k,j)
        RQVCUTEN(i,k,j)=RQVCUTEN(i,k,j)+pattern_isppt_conv(i,j,kh)*RQVCUTEN(i,k,j)
      ENDDO
      ENDDO
      ENDDO
    ENDIF
...
!$omp& private(i,iendx,j,ncubot,ncutop,pcpcol)
      ...
      pcp_cloud: DO J=max(jds+( 0 ),jts-( 0  )),min(jde-( 0 ),jte+( 0  ))
        IENDX=min(ide-( 0 ),ite+( 0  ))
        IF(MOD(J,2)==0.AND.ITE==IDE-1)IENDX=IENDX-1
        DO I=max(ids+( 0 ),its-( 0  )),IENDX
...
!$omp& private(dqdt,dtdt,i,iendx,j,k,tchange)
      ...
      DO J=max(jds+( 2 ),jts-( 0  )),min(jde-( 2 ),jte+( 0  ))
        ...
        DO I=max(ids+( 1 ),its-( 0  )),IENDX
          ...
          CASE (KFSCHEME,KFETASCHEME,GDSCHEME,GFSCHEME,SASSCHEME,SCALESASSCHEME,MESO_SAS,OSASSCHEME,KSASSCHEME,NSASSCHEME,TIEDTKESCHEME)
...
!$omp& private(i,j,k)
     ...
     &                                  ,TBPVS_STATE,TBPVS0_STATE
      ...
      REAL,DIMENSION(IMS:IME,JMS:JME,KMS:KME),INTENT(INOUT) :: qv_ins 
      REAL :: QOLD  
...
!$omp& private(dpl,i,j,k,plyr,ql,tl)
     ...
     &        , QNH_CURR=SCALAR_TRANS(ims,kms,jms,P_QNH), F_QNH=F_QNH          & 
     &        , QVOLG_CURR=SCALAR_TRANS(ims,kms,jms,P_QVOLG), F_QVOLG=F_QVOLG    & 
     &        , QVOLH_CURR=SCALAR_TRANS(ims,kms,jms,P_QVOLH), F_QVOLH=F_QVOLH    & 
...
!$omp parallel do                                                       &
!$omp& private(i,iendx,j,k,tnew,qold)
      DO K=KTS,KTE
        DO J=max(jds+( 2 ),jts-( 0  )),min(jde-( 2 ),jte+( 0  ))
          IENDX=min(ide-( 1 ),ite+( 0  ))
          ...
          DO I=max(ids+( 1 ),its-( 0  )),IENDX
            ...
            qv_ins(I,J,K)=(Q(I,J,K)-QOLD)*RDTPHS
          ENDDO
        ENDDO
      ENDDO

   IF (grid%isppt_mp==1) THEN 
     CALL perturb_mp_tend(grid%isppt_mp,grid%pattern_isppt_mp,                     &
                          grid%stddev_cutoff_isppt_mp,grid%gridpt_stddev_isppt_mp, &
                          grid%t,grid%train_ins,grid%nphs,grid%dt,                 &
                          grid%q,grid%qv_ins,                                      &
                          config_flags,                                            &
                          ids, ide, jds, jde, kds, kde,                            &
                          ims, ime, jms, jme, kms, kme,                            &
...
!$omp& private(i,j,k)
     ...
     &                                                     ,F_RAIN
      ...
      DO I=max(ids+( 0 ),its-( 0  )),min(ide-( 0 ),ite+( 0  ))
        ...
        F_ice(I,K,J)=max(0.0,min(1.0,F_ice(I,K,J)))
        F_rain(I,K,J)=max(0.0,min(1.0,F_rain(I,K,J)))
