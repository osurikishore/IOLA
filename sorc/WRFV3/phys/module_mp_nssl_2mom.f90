...
MODULE module_mp_nssl_2mom
  ...
  real, private    :: icefallfac = 1.0 
  real, private    :: snowfallfac = 1.0 
        ...
        IF ( ipconc .ge. 4 ) THEN
         if ( mixedphase .and. qsvtmod ) then
          ...
          ENDIF
           IF ( infdo  >= 2 ) THEN
            IF ( isnowfall == 1 ) THEN
             vtxbar(mgs,ls,3) = 6.12217*rhovt(mgs)*(xv(mgs,ls))**(1./12.) 
            ELSEIF ( isnowfall == 2 ) THEN
             vtxbar(mgs,ls,3) = 13.3436*rhovt(mgs)*(xv(mgs,ls))**(0.14)   
            ENDIF
           ENDIF
         endif
        ELSE 
...
29998 continue
 ...
 2200 continue
   ...
   END SUBROUTINE NUCOND
     ...
     &             ((3.+alpharaut)*(2.+alpharaut)*(1.+alpharaut))
      ...
      snowmeltmass = pi/6.0 * 1000. * snowmeltdia**3  
 ...
 1100 continue
     ...
     &           ((0.5e20*rb(mgs)**3*xdia(mgs,lc,1))-0.4) )
      ...
      do mgs = 1,ngscnt
      qvimxd(mgs) = 0.70*(qx(mgs,lv)-qis(mgs))*dtpinv 
      
      IF ( qx(mgs,lc) < qxmin(lc) ) qvimxd(mgs) = 0.99*(qx(mgs,lv)-qis(mgs))*dtpinv 
     ...
     &  , qrmxd(mgs))
      ...
      ENDIF
          IF ( lvol(lh) .gt. 1 .or. lh .gt. 1 ) THEN 
     ...
     &            ((1. + alpha(mgs,lr))*(2. + alpha(mgs,lr))*(3. + alpha(mgs,lr)))
         ...
         qrfrz(mgs) = Min( qrfrz(mgs), 1.*qx(mgs,lr)*dtpinv ) 
         crfrz(mgs) = Min( crfrz(mgs), 1.*cx(mgs,lr)*dtpinv ) 
     ...
     &  *rho0(mgs)*(qx(mgs,lc)**2)
      ...
      qwfrz(mgs) = min(qwfrz(mgs),qcmxd(mgs))
       ...
       ELSEIF ( ipconc .ge. 2 ) THEN
         ...
         IF (  cnu == 0.0 ) THEN
         cwfrz(mgs) = cx(mgs,lc)*Exp(-volt/xv(mgs,lc))*dtpinv 
     ...
     IF ( chmlr(mgs) < 0.0 .and. (ibinhmlr < 1 .or. lzh < 1) ) THEN 
     ...
     &    fvds(mgs)*cx(mgs,lh)*hwvent(mgs)*hwcap(mgs)*depfac
      ...
      if( dqwv(mgs) .lt. 0. ) then           
       ...
       qvptmp = qvptmp - ( dqcw(mgs) + dqci(mgs) )  
     ...
     &   + fwet2(mgs)*(qhlaci(mgs) + qhlacs(mgs)) )
      ...
      if ( temg(mgs) .gt. tfr ) then
       ...
       qsshr(mgs)   = - qsacr(mgs) - qsacw(mgs) 
       qhlshr(mgs)  = - qhlacw(mgs) - qhlacr(mgs) 
       qhshr(mgs)  = - qhacw(mgs) - qhacr(mgs) 
     ...
     &                /(temg(mgs)-273.15))**(rimc2)
      ...
      DO mgs = 1,ngscnt
        ...
        IF ( hlcnhdia > 0 ) THEN
          ltest = xdia(mgs,lh,3) .gt. hlcnhdia  
        ELSE 

          ltest =  xdia(mgs,lh,1)*(4. + alpha(mgs,lh)) > Abs( hlcnhdia ) 
        ENDIF
     ...
     &  /((dzfacp+dzfacm))
      ...
      IF ( icenucopt /= -10 ) THEN
        ...
        IF ( lcin > 1 ) THEN
          ciint(mgs) = Min(ciint(mgs), ccin(mgs)*dtpinv) 
     ...
     &  - il5(mgs)*chlcnh(mgs)
      ...
      IF ( lhl .gt. 1 ) THEN 
      do mgs = 1,ngscnt
      pchli(mgs) = (1.0-ifrzg)*(crfrzf(mgs)) & 
