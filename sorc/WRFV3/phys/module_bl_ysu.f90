...
contains
   ...
   subroutine ysu(u3d,v3d,th3d,t3d,qv3d,qc3d,qi3d,p3d,p3di,pi3d,               &
                  ...
                  u10,v10,                                                     &
                  uoce,voce,                                                   &
                  rthraten,ysu_topdown_pblmix, ysu_firstorder, ysu_brcr_ub,    & 
                  ysu_cgrad_scalar, ysu_cgrad_mom,                             & 
                  ctopo,ctopo2,                                                &
                  DKU3D,DKT3D,                                                 & 
   ...
   integer,  intent(in)      ::      ysu_topdown_pblmix, ysu_firstorder, ysu_cgrad_scalar, ysu_cgrad_mom, sfclay  
   real,     intent(in   )   ::      ysu_brcr_ub 
   ...
   real,     dimension( its:ite )  ::                                          &
                                                                        ...
                                                                        dtsfc, &
                                                                        dqsfc
    real,     dimension( its:ite, kts:kte )  ::                      dku,dkt       
  ...
  203 format(1x,i4,1x,i2,10(1x,e15.8))
      ...
      call ysu2d(J=j,ux=u3d(ims,kms,j),vx=v3d(ims,kms,j)                       &
              ...
              ,exch_hx=exch_h(ims,kms,j),exch_mx=exch_m(ims,kms,j)             & 
              ,wstar=wstar(ims,j)                                              &
              ,delta=delta(ims,j)                                              &
              ,u10=u10(ims,j),v10=v10(ims,j)                                   &
              ,xDKU=dku(its,kts),xDKT=dkt(its,kts)  & 
              ,uox=uoce(ims,j),vox=voce(ims,j)                                 &
              ,rthraten=rthraten(ims,kms,j),p2diORG=p3di(ims,kms,j)            &
              ,ysu_topdown_pblmix=ysu_topdown_pblmix                           &
              ,ysu_firstorder=ysu_firstorder                                   & 
              ,ysu_brcr_ub=ysu_brcr_ub                                         & 
              ,ysu_cgrad_scalar=ysu_cgrad_scalar                               & 
              ,ysu_cgrad_mom=ysu_cgrad_mom                                     & 
              ,gz1oz0=gz1oz0(ims,j),sfclay=sfclay                              & 
   ...
   subroutine ysu2d(j,ux,vx,tx,qx,p2d,p2di,pi2d,                               &
                  ...
                  uox,vox,                                                     &
                  rthraten,p2diORG,                                            &
                  ysu_topdown_pblmix,   ysu_firstorder, ysu_brcr_ub,           & 
                  ysu_cgrad_scalar, ysu_cgrad_mom,                             & 
                  gz1oz0, sfclay,                                              & 
                  ctopo,ctopo2,                                                &
                  xDKU,xDKT,                                                   & 
   ...
   do i = its,ite
     ...
     brup(i) = br(i)
     brcr(i) = brcr_ub
     if(ysu_brcr_ub.ne.0.) brcr(i) = ysu_brcr_ub 
   enddo
   ...
   do i = its,ite
    ...
    if(sfclay.ne.88)then 
     ...
     if(sfclay.eq.88)then 
      ust(i) = abs( karman*amax1(wspd1(i),0.01)/fm) 
     endif 
     ust3(i)   = ust(i)**3.
   ...
   do k = kts,kte-1
     do i = its,ite
       if(k.ge.kpbl(i)) then
         ...
         if(ysu_firstorder.gt.1)then  
          rlamdz = min(max(0.1*dza(i,k+1),real(ysu_firstorder)),300.) 
         endif
   ...
   do k = kts,kte-1
     do i = its,ite
       ...
       if(pblflg(i).and.k.lt.kpbl(i)) then
         ...
         if(ysu_cgrad_scalar.eq.0) dsdzt = tem1*(-hfxpbl(i)*zfacent(i,k)/xkzh(i,k)) 
