...
MODULE MODULE_MP_MORR_TWO_MOMENT_AERO
  ...
  use module_model_constants, ONLY: CP, G, R => r_d, RV => r_v, EP_2, t0 
   ...
   PRIVATE :: MORR_TWO_MOMENT_MICRO
     ...
     REAL, PRIVATE :: CONS41
      ...
      real, private:: alogsig_pamdm(naer_cu) 
      real, private:: exp45logsig_pamdm(naer_cu)
      real, private:: argfactor_pamdm(naer_cu)
      real, private:: amcube_pamdm(naer_cu) 
      real, private:: smcrit_pamdm(naer_cu) 
      real, private:: lnsm_pamdm(naer_cu) 
      real, private:: amcubefactor_pamdm(naer_cu) 
      real, private:: smcritfactor_pamdm(naer_cu) 
      real, private:: alog2_pamdm,alog3_pamdm,alogaten_pamdm,surften_pamdm

      real, private:: f1_pamdm(naer_cu),f2_pamdm(naer_cu) 
      real, private:: third_pamdm, sat_pamdm
      real, private:: sq2_pamdm, arg_pamdm
      integer, parameter:: psat_pamdm=7 
      real, private:: super_pamdm(psat_pamdm)
      real, private, parameter :: supersat_pamdm(psat_pamdm)= &
               (/0.02,0.05,0.1,0.2,0.3,0.5,1.0/)
      real, private:: ccnfact_pamdm(psat_pamdm,naer_cu)  
...
CONTAINS
 ...
 SUBROUTINE MORR_TWO_MOMENT_INIT_AERO(morr_rimed_ice) 
...
SUBROUTINE MP_MORR_TWO_MOMENT_AERO(ITIMESTEP,                    &
                TH, QV, QC, QR, QI, QS, QG, NI, NS, NR, NG, NC,  & 
                RHO, PII, P, DT_IN, DZ, HT, W, KZH,     &
                RAINNC, RAINNCV, SR,                    &
		SNOWNC,SNOWNCV,GRAUPELNC,GRAUPELNCV,    & 
                refl_10cm,mskf_refl_10cm,diagflag, do_radar_ref,      & 
               ...
               ,no_src_types_cu                         & 
               ,EFCG, EFIG, EFSG, WACT                  & 
               ,CCN1_GS, CCN2_GS, CCN3_GS, CCN4_GS      & 
               ,CCN5_GS, CCN6_GS, CCN7_GS, NR_CU        & 
               ,QR_CU, NS_CU, QS_CU, CU_UAF             & 
   ...
   REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(INOUT):: &
                          qv, qc, qr, qi, qs, qg, ni, ns, nr, TH, NG, NC 
   ...
   REAL , DIMENSION( ims:ime , jms:jme ) , INTENT(IN) ::       ht, CU_UAF 
 
   REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(IN):: &
                   NR_CU,QR_CU,NS_CU,QS_CU
   ...
   REAL, DIMENSION(kts:kte) ::                                                                & 
                            ...
                            NRMSCU1D, NSMSCU1D, QRMSCU1D, QSMSCU1D, & 

                            NRRAD, QRRAD, NSRAD, QSRAD
  ...
  integer :: iinum 
   ...
   REAL, DIMENSION(kts:kte) :: dBZ,dBZ2 
  ...
  REAL, DIMENSION(kts:kte, no_src_types_cu) :: maerosol, naer
   ...
   DO K=KTS,KTE
       ...
       WVAR(I,K,J) = MAX(0.10,WVAR(I,K,J))
   ...
   do j=jts,jte      
      ...
      do k=kts,kte   
          ...
          CCN6_GS(i,k,j) = 0.0
          CCN7_GS(i,k,j) = 0.0
          NRMSCU1D(k) = NR_CU(i,k,j)
          NSMSCU1D(k) = NS_CU(i,k,j)
          QRMSCU1D(k) = QR_CU(i,k,j)
          QSMSCU1D(k) = QS_CU(i,k,j)
          NRRAD(k) = 0.0
          QRRAD(k) = 0.0
          NSRAD(k) = 0.0
          QSRAD(k) = 0.0
   ...
   ELSE
      ...
      IF (aercu_opt.eq.2) THEN
         ...
         INUC = 2
         maerosol(k,1) = aercu_fct*aerocu(i,k,j,6)*1E-9
         naer(k,1) = 5.64259e13 * maerosol(k,1)**0.58

         maerosol(k,2) = aercu_fct*aerocu(i,k,j,5)*1E-9
         naer(k,2) = maerosol(k,2)*num_to_mass_aer(2)

         maerosol(k,3) = aercu_fct*1.44*aerocu(i,k,j,1)*1E-9
         naer(k,3) = maerosol(k,3)*num_to_mass_aer(3)

         maerosol(k,4) = aercu_fct*1.44*aerocu(i,k,j,2)*1E-9
         naer(k,4) = maerosol(k,4)*num_to_mass_aer(4)

         maerosol(k,5) = aercu_fct*1.44*aerocu(i,k,j,3)*1E-9
         naer(k,5) = maerosol(k,5)*num_to_mass_aer(5)

         maerosol(k,6) = aercu_fct*1.44*aerocu(i,k,j,4)*1E-9
         naer(k,6) = maerosol(k,6)*num_to_mass_aer(6)

         maerosol(k,7) = aercu_fct*1.54*aerocu(i,k,j,9)*1E-9
         naer(k,7) = maerosol(k,7)*num_to_mass_aer(7)

         maerosol(k,8) = aercu_fct*1.37*aerocu(i,k,j,7)*1E-9
         naer(k,8) = maerosol(k,8)*num_to_mass_aer(8)

         maerosol(k,9) = aercu_fct*1.25*aerocu(i,k,j,10)*1E-9
         naer(k,9) = maerosol(k,9)*num_to_mass_aer(9)

         maerosol(k,10) = aercu_fct*1.37*aerocu(i,k,j,8)*1E-9
   ...
   ENDIF
      ...
      SR(i,j) = SNOWRT1D/(PRECPRT1D+1.E-12)
         ...
         if (diagflag .and. do_radar_ref == 1) then
          ...
          NRRAD = nr1d + (NRMSCU1D*CU_UAF(i,j)) 
          QRRAD = qr1d + (QRMSCU1D*CU_UAF(i,j)) 
          NSRAD = ns1d + (NSMSCU1D*CU_UAF(i,j)) 
          QSRAD = qs1d + (QSMSCU1D*CU_UAF(i,j)) 
          call refl10cm_hm (qv1d, QRRAD, NRRAD, QSRAD, NSRAD, qg1d, ng1d,   & 
                      t1d, p1d, dBZ2, kts, kte, i, j)
          do k = kts, kte
             mskf_refl_10cm(i,k,j) = MAX(-35., dBZ2(k))
          enddo
         endif
         ENDIF
...
END SUBROUTINE MP_MORR_TWO_MOMENT_AERO
 ...
 300  CONTINUE
      ...
      kc2=MAX(kc2/rho(k),0.)  
         ...
         if ((QVQVS(K).GE.0.999.and.T3D(K).le.265.15).or. &
            ...
            DUM = W3D(K)+WVAR(K)
            call mdm_prescribed_nucleati(DUM,T3D(K),QVQVS(K),QVQVSI(K),QC3D(K),RHO(K),  &  
                        naero,naer_cu,KC2 &
                        , KC2H,     &
                        KC2IM,KC2D,KC2DM)
 ...
 195  continue
  ...
  subroutine mdm_prescribed_activate(wbar, tair, rhoair,  &
      ...
      etafactor2max=1.e10/(alpha_cesm*wbar)**1.5 

      do m=1,nmode

          volc(m)=maero(m)/(rhodry(m)) 
         if(volc(m).gt.1.e-39.and.naero(m).gt.1.e-39)then
            etafactor2(m)=1./(naero(m)*beta*sqrtg)  

            amcubeloc(m)=(3.*volc(m)/(4.*pi*exp45logsig_pamdm(m)*naero(m)))  
            smc(m)=smcrit_pamdm(m) 
         ...
         wnuc=wbar
            ...
            do m=1,nmode
               x=2*(lnsmloc(m)-lnsmax)/(3*sq2_pamdm*alogsig_pamdm(m))
                nact=nact+0.5*(1.-DERF1(x))*naero(m)  
            enddo
            nact=nact/rhoair 
...
subroutine mdm_prescribed_nucleati(wbar, tair, relhum, relhumi,  qc,  rhoair, &
       naero,  naer_all, nuci  &
  ...
  real  so4_num                                      
  real  soot_num                                     
  real  dst1_num,dst2_num,dst3_num,dst4_num          
  real  dst_num                                      
  real  nihf                                         
  real  niimm                                        
  real  nidep                                        
  ...
  real  dmc,ssmc         
    ...
    dst4_num = 0.0
      ...
      so4_num=naero(1)*1.0e-6 
      soot_num=naero(10)*1.0e-6 
      dst1_num=naero(3)*1.0e-6 
      dst2_num=naero(4)*1.0e-6 
      dst3_num=naero(5)*1.0e-6 
      dst4_num=naero(6)*1.0e-6 
  ...
  if((tc.le.-35.0).and.((relhumi*subgrid).ge.1.2))then   
       ...
       if(tc.gt.regm) then    
         if(tc.lt.-40. .and. wbar.gt.1.) then 
...
1100  continue
    ...
    if(relhumi.gt.1.5) then 
        deles = 1.5         
    else  
        deles = relhumi
    end if 

      nimey=1.e-3*exp(12.96*(deles-1.0) - 0.639)   
    else
      nimey=0.
    endif
    ...
    if(nuci.gt.9999..or.nuci.lt.0.) then
       ...
       write(*, *) ni, tair, relhum, relhumi, wbar, nihf,niimm,nidep,nimey,dst2_num,dst3_num,dst4_num
       nuci=0.
         stop 'nuclei prbolem?'
    endif

    nuci=nuci*1.e+6/rhoair    
