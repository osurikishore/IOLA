...
CONTAINS
   ...
   USE module_cu_gfdrvr , ONLY : gfdrvr         
...
wrf_err_message )
          ...
          CALL CU_SCALESAS(                                     &
               ...
               ,P_QI=p_qi,P_FIRST_SCALAR=param_first_scalar     &
               ,DX2D=dx2d,DY=dynmm                              & 
               ,SCALEFUN=SCALEFUN,SCALEFUN1=SCALEFUN1           & 
               ,SIGMU=SIGMU,SIGMU1=SIGMU1                       & 
               ,IDS=ids,IDE=ide,JDS=jds,JDE=jde,KDS=kds,KDE=kde &
               ,IMS=ims,IME=ime,JMS=jms,JME=jme,KMS=kms,KME=kme &
               ,ITS=its,ITE=ite,JTS=jts,JTE=jte,KTS=kts,KTE=kte &
                                                                )
...
SUBROUTINE aer_p_int_cu(p ,pin_cu, levsiz_cu, aerotcu, aerocu, no_src_cu, pf, &
   ...
   integer, intent(in) :: levsiz_cu              
   integer, intent(in) :: no_src_cu              

   real, intent(in) :: p(ims:ime,kms:kme,jms:jme)
   real, intent(in) :: pf(ims:ime,kms:kme,jms:jme)
   real, intent(in) :: pin_cu(ims:ime,levsiz_cu,jms:jme)        
   real, intent(in) :: aerotcu(ims:ime,levsiz_cu,jms:jme,no_src_cu) 
   real, intent(out) :: aerocu(ims:ime,kms:kme,jms:jme,no_src_cu+1) 
