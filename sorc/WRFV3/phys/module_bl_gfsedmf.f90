...
CONTAINS
   ...
   SUBROUTINE BL_GFSEDMF(U3D,V3D,TH3D,T3D,QV3D,QC3D,QI3D,P3D,PI3D, &
      ...
      real :: VAR_RIC,coef_ric_l,coef_ric_s,alpha,alpha_in
      real(kind=kind_phys)removedis
      logical lprnt
      integer ipr
   ...
   DO J=jts,jte
     ...
     &   kinver,xkzm_m,xkzm_h,xkzm_s,lprnt,ipr,zol,            &
         pert_pbl, ens_random_seed, ens_pblamp,             &
     &   VAR_RIC,coef_ric_l,coef_ric_s,alpha_in,xland1,removedis)
   ...
   END SUBROUTINE gfsedmfinit
     ...
     &   kinver,xkzm_m,xkzm_h,xkzm_s,lprnt,ipr,zol,                     &
           pert_pbl, ens_random_seed, ens_pblamp,             &
     &   VAR_RIC,coef_ric_l,coef_ric_s,alpha,xland1,removedis)
     ...
     &                     a2(im,km*ntrac)
       ...
       real(kind=kind_phys) dkt1(im,km-1),dku1(im,km-1)      
       real(kind=kind_phys) removedis                        
     ...
     real :: smax,ashape,sz2h, sksfc,skmax,ashape1,skminusk0, hmax
      ...
      real*8 :: ran1           
      real :: rr
      logical,save :: pert_pbl_local 
      integer,save :: ens_random_seed_local,env_pp_local
      integer :: ensda_physics_pert 
      real,save :: ens_pblamp_local   
     ...
     &              (g*zl(i,k)/thvx(i,1))/spdk2
             ...
             if(useshape .ge. 1) then
                ...
                sz2h=min(sz2h,1.0)
                    zfac=(1.0-sz2h)**pfac
                     ...
                     sksfc=min(ZI(I,2)/HPBL(I),0.05)  
                     sksfc=sksfc*(1-sksfc)**pfac

                zfac=max(zfac,zfmin)
                ashape=max(ABS(ALPHA),0.2)  
                if(useshape ==1) then 
      ...
      do k = 1, km1
         do i=1,im
            ...
            if(k >= int( kpbl(i)*removedis) ) then   
               ...
               if(ri < 0.) then 
                  ...
                  dku1(i,k) = dk*(1+8.*(-ri)/(1+1.746*sri))
                  dkt1(i,k) = dk*(1+8.*(-ri)/(1+1.286*sri))
               else             
                  rl2      = zk*rlam/(rlam+zk)
                ...
                dku1(i,k) = min(dku1(i,k),dkmax)
                dku1(i,k) = max(dku1(i,k),xkzmo(i,k))
                dkt1(i,k) = min(dkt1(i,k),dkmax)
                dkt1(i,k) = max(dkt1(i,k),xkzo(i,k))
                 if ( removedis < 1.0 ) then
                  dku(i,k)=max(dku1(i,k),dku(i,k))
                  dkt(i,k)=max(dkt1(i,k),dkt(i,k))
                 else
                  dku(i,k)=dku1(i,k)
                  dkt(i,k)=dkt1(i,k)
                 endif
