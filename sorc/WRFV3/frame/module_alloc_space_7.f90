...
IF ( setinitval .EQ. 3 ) grid%ocean_cpl=.FALSE.
IF ( setinitval .EQ. 3 ) grid%num_moves=0
IF ( setinitval .EQ. 3 ) grid%vortex_interval=0
IF ( setinitval .EQ. 3 ) grid%corral_dist=0
IF ( setinitval .EQ. 3 ) grid%multi_storm=.FALSE.
IF ( setinitval .EQ. 3 ) grid%no_ocean=.FALSE.
...
IF ( setinitval .EQ. 3 ) grid%ysu_firstorder=0
IF ( setinitval .EQ. 3 ) grid%ysu_brcr_ub=initial_data_value
IF ( setinitval .EQ. 3 ) grid%ysu_cgrad_scalar=0
IF ( setinitval .EQ. 3 ) grid%ysu_cgrad_mom=0
...
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'o3rad').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
...
ENDIF
IF ( setinitval .EQ. 3 ) grid%ntrack=0
...
IF ( setinitval .EQ. 3 ) grid%nhmove=0
...
IF ( setinitval .EQ. 3 ) grid%dwdt_damping_lev=initial_data_value
