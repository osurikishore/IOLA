...
SUBROUTINE HALO_NMM_INIT_40_sub ( grid, &
  local_communicator, &
  mytask, ntasks, ntasks_x, ntasks_y, &
  ids, ide, jds, jde, kds, kde,       &
  ims, ime, jms, jme, kms, kme,       &
...
CALL wrf_debug(2,'calling inc/HALO_NMM_INIT_40_inline.inc')
CALL rsl_comm_iter_init(5,jps,jpe)
DO WHILE ( rsl_comm_iter( grid%id , grid%is_intermediate, 5 , &
                         0 , jds,jde,jps,jpe, grid%njds, grid%njde , & 
     rsl_sendbeg_m, rsl_sendw_m, rsl_sendbeg_p, rsl_sendw_p,   & 
...
IF ( SIZE(grid%dthcudt,1)*SIZE(grid%dthcudt,2) .GT. 1 ) THEN
CALL RSL_LITE_PACK ( local_communicator,&
 grid%dthcudt, 5,&
rsl_sendbeg_m, rsl_sendw_m, rsl_sendbeg_p, rsl_sendw_p, &
rsl_recvbeg_m, rsl_recvw_m, rsl_recvbeg_p, rsl_recvw_p, &
...
ips, ipe, jps, jpe, kps, kpe              )
ENDIF
IF ( SIZE(grid%dqvcudt,1)*SIZE(grid%dqvcudt,2) .GT. 1 ) THEN
CALL RSL_LITE_PACK ( local_communicator,&
 grid%dqvcudt, 5,&
rsl_sendbeg_m, rsl_sendw_m, rsl_sendbeg_p, rsl_sendw_p, &
rsl_recvbeg_m, rsl_recvw_m, rsl_recvbeg_p, rsl_recvw_p, &
...
ips, ipe, jps, jpe, kps, kpe              )
ENDIF
IF ( SIZE(grid%dqccudt,1)*SIZE(grid%dqccudt,2) .GT. 1 ) THEN
CALL RSL_LITE_PACK ( local_communicator,&
 grid%dqccudt, 5,&
rsl_sendbeg_m, rsl_sendw_m, rsl_sendbeg_p, rsl_sendw_p, &
rsl_recvbeg_m, rsl_recvw_m, rsl_recvbeg_p, rsl_recvw_p, &
...
CALL pop_communicators_for_domain
  ...
  END SUBROUTINE HALO_NMM_INIT_40_sub
