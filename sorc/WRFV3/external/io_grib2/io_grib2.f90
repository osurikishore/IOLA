...
! For 1 only
  ...
  if(WrfIOnotInitialized) then
    ...
    write(msg,*) 'ext_gr2_ioinit was not called ',"io_grib2.F",', line', 466
...
subroutine gr2_addgrid_w(cgrib, lcgrib, central_lat, central_lon, wrf_projection, &
  ...
  real, parameter                           :: DEG_TO_RAD = 3.141593/180
  real, parameter                           :: RAD_TO_DEG = 180/3.141593
  ...
  !
  if ((wrf_projection .eq. 0) &
       .or. (wrf_projection .eq. 6)) then
