   ...
   CALL med_nest_egrid_configure ( parent , nest )
    ...
    IF ( nest%active_this_task .OR. parent%active_this_task ) THEN
      IF ( parent%active_this_task ) THEN
        CALL BYTE_BCAST( parent%PSTD, KME*4, mpi_comm_to_kid( which_kid( nest%id ) , parent%id ) )
      ELSE
        CALL BYTE_BCAST( nest%PSTD, KME*4, mpi_comm_to_mom( nest%id ) )
      ENDIF
    ENDIF
    ...
    IF ( nest%active_this_task ) THEN
      ...
      DO J = JPS, MIN(JPE,JDE-1)
        DO I = IPS, MIN(IPE,IDE-1)
         ...
         nest%sm(I,J)=1-nest%hres_lnd(I,J)
         nest%lu_mask(I,J)=nest%sm(I,J)
         nest%landmask(I,J)=nest%hres_lnd(I,J)
         nest%lu_index(I,J)=nest%hres_lu_index(I,J)
       ENDDO
      ENDDO
...
write(0,*)"mediation_integrate.G",1947,trim(n2)
write(0,*)"mediation_integrate.G",1948,' grid%id ',grid%id,' grid%oid ',grid%oid
...
message )
   ...
   CALL construct_filename2a ( fname , hist_outname, &
                               grid%id , 2 , timestr )
   stream_l = stream-first_history+1
   IF ( stream_l .EQ. history_only ) THEN
     WRITE(n2,'("DATASET=HISTORY")')
   ELSE IF ( stream_l-1 .GE. 10 ) THEN
     WRITE(n2,'("DATASET=AUXHIST",I2)')stream_l-1
   ELSE
     WRITE(n2,'("DATASET=AUXHIST",I1)')stream_l-1
   ENDIF
   IF ( oid .eq. 0 ) THEN
