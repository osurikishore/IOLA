     ...
     DO J=NJTS,MIN(NJTE,NJDE-1)
      DO I=NITS,MIN(NITE,NIDE-1)
          ...
          DO N=1,4
             ...
             ENDIF
             SUM=SUM+NBWGT(N,I,J)
             IF(SUM .GT. 1.0)CALL wrf_error_fatal3("<stdin>",624,&
"horizontal interp error - interp_hnear_nmm" )
  ...
  SUBROUTINE force_sppt_nmm    (cfld,                                 &  
                               cids, cide, ckds, ckde, cjds, cjde,   &
                               cims, cime, ckms, ckme, cjms, cjme,   &
                               cits, cite, ckts, ckte, cjts, cjte,   &
                               nfld,                                 &  
     ...
     INTEGER  i,j,k,n
     REAL     SUM,AMAXVAL
     REAL,    DIMENSION (4, nims:nime, njms:njme, nkms:nkme ) :: NBWGT
    ...
    DO K=NKTS,NKTE
     DO J=NJTS,MIN(NJTE,NJDE-1)
      DO I=NITS,MIN(NITE,NIDE-1)
            NBWGT(1,I,J,K)=HBWGT1(I,J)
            NBWGT(2,I,J,K)=HBWGT2(I,J)
            NBWGT(3,I,J,K)=HBWGT3(I,J)
            NBWGT(4,I,J,K)=HBWGT4(I,J)
      ENDDO
     ENDDO
    ENDDO
    ...
    DO K=NKTS,NKTE
     DO J=NJTS,MIN(NJTE,NJDE-1)
      DO I=NITS,MIN(NITE,NIDE-1)
          AMAXVAL=0.
          DO N=1,4
            AMAXVAL=amax1(NBWGT(N,I,J,K),AMAXVAL)
          ENDDO

          FLIP=.TRUE.
          SUM=0.0
          DO N=1,4
             IF(AMAXVAL .EQ. NBWGT(N,I,J,K) .AND. FLIP)THEN
               NBWGT(N,I,J,K)=1.0
               FLIP=.FALSE.
             ELSE
               NBWGT(N,I,J,K)=0.0
             ENDIF
             SUM=SUM+NBWGT(N,I,J,K)
             IF(SUM .GT. 1.0)CALL wrf_error_fatal3("<stdin>",740,&
"horizontal interp error - interp_hnear_nmm" )
          ENDDO
      ENDDO
     ENDDO
    ENDDO
      ...
      DO K=NKTS,NKTE
       DO J=NJTS,MIN(NJTE,NJDE-1)
        DO I=NITS,MIN(NITE,NIDE-1)
            IF(MOD(JJH(I,J),2) .NE. 0)THEN    
                NFLD(I,J,K) = NBWGT(1,I,J,K)*CFLD(IIH(I,J),  JJH(I,J), K  ) &
                          + NBWGT(2,I,J,K)*CFLD(IIH(I,J)+1,JJH(I,J),K  ) &
                          + NBWGT(3,I,J,K)*CFLD(IIH(I,J),  JJH(I,J)-1, K) &
                          + NBWGT(4,I,J,K)*CFLD(IIH(I,J),  JJH(I,J)+1, K)
            ELSE
                NFLD(I,J,K) = NBWGT(1,I,J,K)*CFLD(IIH(I,J),  JJH(I,J), K  ) &
                          + NBWGT(2,I,J,K)*CFLD(IIH(I,J)+1,JJH(I,J), K  ) &
                          + NBWGT(3,I,J,K)*CFLD(IIH(I,J)+1,JJH(I,J)-1, K) &
                          + NBWGT(4,I,J,K)*CFLD(IIH(I,J)+1,JJH(I,J)+1, K)
            ENDIF
        ENDDO
       ENDDO
      ENDDO

     END SUBROUTINE force_sppt_nmm
   ...
   subroutine ForceNearMaskSST      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
        ...
        cactivate, nactivate)
     use module_interp_store
     use module_interp_nmm, only: c2n_nearmask2d,c2n_nearmask3d
     implicit none
     LOGICAL,INTENT(IN) :: xstag, ystag
     INTEGER,INTENT(IN) :: cids, cide, ckds, ckde, cjds, cjde,   &
          cims, cime, ckms, ckme, cjms, cjme,   &
          ...
          nims, nime, nkms, nkme, njms, njme,   &
          nits, nite, nkts, nkte, njts, njte,   &
          shw,ipos,jpos,nri,nrj,cflag,nflag
     ...
     REAL,DIMENSION(cims:cime,cjms:cjme),INTENT(IN)           :: clu
     REAL,DIMENSION(nims:nime,njms:njme),INTENT(IN)           :: nlu

     INTEGER, INTENT(IN), DIMENSION(*) :: cactivate, nactivate

     REAL                     :: defvalyes, defvalno
     ...
     if(nkts==nkte) then
        call c2n_nearmask2d(hnear_i,hnear_j,IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
        ...
        call c2n_nearmask3d(hnear_i,hnear_j,IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,           &
             cims, cime, cjms, cjme, ckms, ckme,   &
             nids, nide, njds, njde, nkds, nkde,   &
   ...
   end subroutine ForceNearMaskSST

   subroutine DownNearMask      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
   ...
   end subroutine DownNearMask

   subroutine DownNearMaskDef      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
     ...
     REAL,DIMENSION(cims:cime,cjms:cjme),INTENT(IN)           :: clu
     REAL,DIMENSION(nims:nime,njms:njme),INTENT(IN)           :: nlu
     REAL,INTENT(IN)          :: defvalyesopt, defvalnoopt

     REAL                     :: defvalyes, defvalno

     defvalyes=defvalyesopt
     defvalno=defvalnoopt
   ...
   end subroutine DownNearMaskDef

   subroutine DownNearMaskIKJ      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
        ...
        clu, nlu, cflag, nflag)
     use module_interp_store
     use module_interp_nmm, only: c2n_nearmask3dikj
     implicit none
     LOGICAL,INTENT(IN) :: xstag, ystag
     INTEGER,INTENT(IN) :: cids, cide, ckds, ckde, cjds, cjde,   &
          cims, cime, ckms, ckme, cjms, cjme,   &
...
'IJ interpolation of an IKJ variable is not supported and makes no sense anyway. Use DownNearMask instead.')

     else
        call c2n_nearmask3dikj(hnear_i,hnear_j,IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,          &
             cims, cime, cjms, cjme, ckms, ckme,   &
             nids, nide, njds, njde, nkds, nkde,   &
   ...
   end subroutine DownNearMaskIKJ

   subroutine DownNearMaskDefIKJ      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
...
'IJ interpolation of an IKJ variable is not supported and makes no sense anyway. Use DownNearMask instead.')
   ...
   end subroutine DownNearMaskDefIKJ
...
'IJ interpolation of an IKJ variable is not supported and makes no sense anyway.  Use DownNear instead.')
   ...
   subroutine UpNear(cfld,                    &  
     ...
     if(nkts/=nkte) &
          call wrf_error_fatal3("<stdin>",1570,&
...
'3D integer nearest neighbor interpolation is not implemented.')
   ...
   subroutine DownINearMask      (cfld,                                 &  
        ...
        clu, nlu, cflag, nflag)
     use module_interp_store
     use module_interp_nmm, only: c2n_inearmask2d
     implicit none
     LOGICAL,INTENT(IN) :: xstag, ystag
     INTEGER,INTENT(IN) :: cids, cide, ckds, ckde, cjds, cjde,   &
          cims, cime, ckms, ckme, cjms, cjme,   &
     ...
     INTEGER,DIMENSION(nims:nime,njms:njme),          INTENT(IN)           :: IMASK
     INTEGER,DIMENSION(cims:cime,cjms:cjme,ckms:ckme),   INTENT(IN)        :: CFLD
     INTEGER,DIMENSION(nims:nime,njms:njme,nkms:nkme),   INTENT(INOUT)     :: nfld

     REAL,DIMENSION(cims:cime,cjms:cjme),INTENT(IN)           :: clu
     REAL,DIMENSION(nims:nime,njms:njme),INTENT(IN)           :: nlu

     INTEGER                     :: defvalyes, defvalno

     defvalyes=-999
     defvalno=-999

     if(nkts==nkte) then
        call c2n_inearmask2d(hnear_i,hnear_j,IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,           &
             cims, cime, cjms, cjme,   &
             nids, nide, njds, njde,   &
...
'3D integer nearest neighbor interpolation is not implemented.')
   ...
   end subroutine DownINearMask

   subroutine DownINearMaskDef      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
     ...
     REAL,DIMENSION(cims:cime,cjms:cjme),INTENT(IN)           :: clu
     REAL,DIMENSION(nims:nime,njms:njme),INTENT(IN)           :: nlu
     INTEGER,INTENT(IN)          :: defvalyesopt, defvalnoopt
...
'3D integer nearest neighbor interpolation is not implemented.')
   ...
   end subroutine DownINearMaskDef
...
'IKJ 2D interpolation of an IJ array is not implemented (and makes no sense anyway).  Use DownCopy instead.')
   ...
   subroutine DownCopyMask      (cfld,                                 &  
        ...
        clu, nlu, cflag, nflag)
     use module_interp_store
     use module_interp_nmm, only: c2n_copymask3d, c2n_copymask2d
     implicit none
     LOGICAL,INTENT(IN) :: xstag, ystag
     INTEGER,INTENT(IN) :: cids, cide, ckds, ckde, cjds, cjde,   &
          cims, cime, ckms, ckme, cjms, cjme,   &
     ...
     if(nkts==nkte) then
        call c2n_copymask2d(IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,           &
             cims, cime, cjms, cjme,   &
             nids, nide, njds, njde,   &
        ...
        call c2n_copymask3d(IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,           &
             cims, cime, cjms, cjme, ckms, ckme,   &
             nids, nide, njds, njde, nkds, nkde,   &
   ...
   end subroutine DownCopyMask

   subroutine DownCopyMaskDef      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
   ...
   end subroutine DownCopyMaskDef

   subroutine DownCopyMaskIKJ      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
        ...
        clu, nlu, cflag, nflag)
     use module_interp_store
     use module_interp_nmm, only: c2n_copymask3dikj
     implicit none
     LOGICAL,INTENT(IN) :: xstag, ystag
     INTEGER,INTENT(IN) :: cids, cide, ckds, ckde, cjds, cjde,   &
          cims, cime, ckms, ckme, cjms, cjme,   &
...
'IJ interpolation of an IKJ variable is not supported and makes no sense anyway. Use DownCopyMask instead.')
     else
        call c2n_copymask3dikj(IIH,JJH,HBWGT1,HBWGT2,HBWGT3,HBWGT4,  &
             cfld,nfld,imask,                &
             clu,nlu,cflag,nflag,defvalyes,defvalno,           &
             cims, cime, cjms, cjme, ckms, ckme,   &
             nids, nide, njds, njde, nkds, nkde,   &
   ...
   end subroutine DownCopyMaskIKJ

   subroutine DownCopyMaskDefIKJ      (cfld,                                 &  
        cids, cide, ckds, ckde, cjds, cjde,   &
        cims, cime, ckms, ckme, cjms, cjme,   &
        cits, cite, ckts, ckte, cjts, cjte,   &
        nfld,                                 &  
...
'IJ interpolation of an IKJ variable is not supported and makes no sense anyway. Use DownCopyMask instead.')
   ...
   end subroutine DownCopyMaskDefIKJ
