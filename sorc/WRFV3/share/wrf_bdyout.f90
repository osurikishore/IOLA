...
CALL wrf_ext_write_field (  &
                       ...
                       'bdy_width'               , &  
                       'one_element'               , &  
                       'bdy Sea mask; =1 for sea, =0 for land'               , &  
                       ''               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BXS memorder XS' , & 
...
CALL wrf_ext_write_field (  &
                       ...
                       'bdy Sea mask; =1 for sea, =0 for land'               , &  
                       ''               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BXE memorder XE' , & 
1, (jde-1), 1, config_flags%spec_bdy_width, 1, 1, &
jms, jme, 1, config_flags%spec_bdy_width, 1, 1, &
jps, MIN( (jde-1), jpe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
CALL wrf_ext_write_field (  &
                       ...
                       'bdy Sea mask; =1 for sea, =0 for land'               , &  
                       ''               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BYS memorder YS' , & 
...
CALL wrf_ext_write_field (  &
                       ...
                       'bdy Sea mask; =1 for sea, =0 for land'               , &  
                       ''               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BYE memorder YE' , & 
1, (ide-1), 1, config_flags%spec_bdy_width, 1, 1, &
ims, ime, 1, config_flags%spec_bdy_width, 1, 1, &
ips, MIN( (ide-1), ipe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
CALL wrf_ext_write_field (  &
                       ...
                       'bdy_width'               , &  
                       'one_element'               , &  
                       'bdy tend Sea mask; =1 for sea, =0 for land'               , &  
                       '()/dt'               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BTXS memorder XS' , & 
1, (jde-1), 1, config_flags%spec_bdy_width, 1, 1, &
jms, jme, 1, config_flags%spec_bdy_width, 1, 1, &
jps, MIN( (jde-1), jpe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
CALL wrf_ext_write_field (  &
                       ...
                       'bdy tend Sea mask; =1 for sea, =0 for land'               , &  
                       '()/dt'               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BTXE memorder XE' , & 
1, (jde-1), 1, config_flags%spec_bdy_width, 1, 1, &
jms, jme, 1, config_flags%spec_bdy_width, 1, 1, &
jps, MIN( (jde-1), jpe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
CALL wrf_ext_write_field (  &
                       ...
                       'bdy tend Sea mask; =1 for sea, =0 for land'               , &  
                       '()/dt'               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BTYS memorder YS' , & 
1, (ide-1), 1, config_flags%spec_bdy_width, 1, 1, &
ims, ime, 1, config_flags%spec_bdy_width, 1, 1, &
ips, MIN( (ide-1), ipe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
CALL wrf_ext_write_field (  &
                       ...
                       'bdy tend Sea mask; =1 for sea, =0 for land'               , &  
                       '()/dt'               , &  
'inc/wrf_bdyout.inc ext_write_field SM_BTYE memorder YE' , & 
1, (ide-1), 1, config_flags%spec_bdy_width, 1, 1, &
ims, ime, 1, config_flags%spec_bdy_width, 1, 1, &
ips, MIN( (ide-1), ipe ), 1, config_flags%spec_bdy_width, 1, 1, &
                       ierr )
