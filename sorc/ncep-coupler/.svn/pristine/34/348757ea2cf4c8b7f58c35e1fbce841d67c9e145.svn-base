...
C
      ...
      INTEGER,INTENT(INOUT):: ierr
                  ...
                  !**ierr=-4 returns if STUCK and abs(ierr)=1 on input**
                  ! in: if data are read from disk (see iiu) and if the
                  ! read input data fail to coincide with the
                  ! corresponding arguments then: if ierr.ge.0 on input
                  ! subroutine will return abnormally; otherwise
                  ! the data will be calculated as if with iiu<0
                  !*** if abs(ierr)=1 on input,     ***
                  !*** STUCK is considered nonfatal ***
...
!     integer,dimension(:),allocatable:: ibnd1,jbnd1
     ...
     >r1r2,dlon2,dlat2,dlon1on2,dlat1on2
      ...
      logical same
      logical stuck_ok,improbable_stuck
C

      ierr_in=ierr
      stuck_ok=abs(ierr).eq.1
      ierr=0
C
...
C        drawn over empty areas.)
      ...
      IF (KM2.eq.0) THEN   !  insertion: all MASK2 negative values OK
        print*,'INTP_INI: KM2=0; IM2,JM2,is,ie,istep,MASK2(1,1): ',
     >  IM2,JM2,is,ie,istep,MASK2(1,1)
        print*,'INTP_INI: Total # of valid G2 points = 0; EXITING'
        RETURN
      END IF
...
! End 3.2.2
     ...
     >      IG1(i2p,j2p)
            if (.not.stuck_ok) RETURN
            inG1=improbable_stuck
            if (.not.improbable_stuck) IG1(i2,j2)=-2
              cycle kloop
          end if
        end do
